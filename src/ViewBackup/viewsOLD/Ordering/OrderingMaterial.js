...
class OrderingMaterial extends Component {
    ...
    //   }
      ...
      handleChangeProject=(e)=>{
        ...
        this.setState({project_select : value, comm_select : "", list_item_comm : [], list_pp : [], pp_selected : new Map()});
        if(value !== ""){
      ...
      handleChangeComm=(e)=>{
        ...
        console.log("getCommercial", value);
        if(value !== ""){
      ...
      async exportOrderingFormatPremium(){
        ...
        for(let i = 0; i < dataMaterial.length; i++){
          ...
          if(qty_po !== 0){
            ws.addRow(["", dataMaterial[i].product_name + " (1)", dataMaterial[i].pp_id, 1]);
            ws.getCell('A'+firstMaterial).value = dataMaterial[i].product_name;
            firstMaterial++;
            for(let j = 1; j < qty_po; j++ ){
              ws.addRow(["", dataMaterial[i].product_name + " ("+(j+1)+")", dataMaterial[i].pp_id, 1]);
              firstMaterial++;
            }
          }
                ...
                <table style={{width : '35%'}} className="table-header">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>:</td>
                      <td style={{paddingLeft:'10px', whiteSpace: 'nowrap'}}>
                        {this.state.commercialData === null ? "" : this.state.commercialData.po_number === null ? (<span style={{color : 'red'}}>please assign PO first</span>) : this.state.commercialData.po_number}
                      </td>
                    </tr>
                    <tr>
...
export default connect(mapStateToProps)(OrderingMaterial);
