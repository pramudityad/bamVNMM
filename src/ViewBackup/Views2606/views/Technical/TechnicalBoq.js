  ...
  exportFormatTechnicalVerticalUploaderCommercial = async () =>{
    ...
    for(let i = 0; i < dataSites.length ; i++){
      ...
      if(this.state.version_selected !== null && dataTech.version !== this.state.version_selected){
        ...
        for(let j = 0; j < dataSites[i].siteItemConfig.length; j++ ){
          ws.addRow([dataSites[i].site_id, dataSites[i].program, dataSites[i].sow, dataSites[i].siteItemConfig[j].config_group, dataSites[i].siteItemConfig[j].config_id, dataSites[i].siteItemConfig[j].config_group_type, dataSites[i].siteItemConfig[j].qty_commercial]);
        }
      }
    }

    const MRFormat = await wb.xlsx.writeBuffer();
    saveAs(new Blob([MRFormat]), 'Technical BOQ '+dataTech.no_tech_boq+'Commercial Vertical Uploader.xlsx');
  }
