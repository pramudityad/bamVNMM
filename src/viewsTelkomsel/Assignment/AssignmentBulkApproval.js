...
class AssignmentBulkApproval extends Component {
  constructor(props) {
    ...
    this.handleChangeFilterUserChecked = this.handleChangeFilterUserChecked.bind(this);
  }
  ...
  async getAssignmentListAll() {
    ...
    filter_array.push('"$or":[{"Current_Status":"REQUEST PM APPROVAL"}]');
    let whereAnd = '{' + filter_array.join(',') + '}';
    const res = await this.getDataFromAPINODE('/aspAssignment/aspassign?noPg=1&srt=_id:-1&q=' + whereAnd + '&v={"_id":1}')
    if (res.data !== undefined) {
      const items = res.data.data;
      returnAll = items;
    }
  ...
  async approvalBulk() {
    ...
    const res = await this.patchDatatoAPINODE('/aspAssignment/pmApprovalMany', { "data": sucPatch })
    if (res.data !== undefined) {
      //Send Email Creator
  ...
  handleChangeFilterUserChecked(e){
    this.setState({check_userall : e.target.checked})
  }
            ...
            <Card>
              ...
              <CardFooter>
                {this.state.data_asg_checked.length !== 0 && (
                  <div>
                    <Button color='success' style={{ float: 'right' }} onClick={this.approvalBulk}> Approve</Button>
                  </div>
                )}
              </CardFooter>
            </Card>
