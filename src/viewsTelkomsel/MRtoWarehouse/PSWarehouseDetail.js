...
import { getDatafromAPITSEL, getDatafromAPINODE, postDatatoAPINODE, patchDatatoAPINODE} from "../../helper/asyncFunction";
...
class PSDisDetail extends Component {
  constructor(props) {
    ...
    this.submitPSWH = this.submitPSWH.bind(this);
  ...
  exportFormatPSDismantle = async () =>{
    ...
    saveAs(new Blob([MRFormat]), 'PS Warehouse Template.xlsx');
  }

  componentDidMount(){
    this.getDataPSDis(this.props.match.params.id);
    document.title = "PS WH Detail | BAM"
  }

  async getDataPSDis(_id){
    const res = await getDatafromAPINODE('/plantspec/'+_id, this.props.dataLogin.token);
    if(res !== undefined && res.data !== undefined){
      this.setState({ps_wh_data : res.data.data});
    }
  }
  ...
  async loadOptionsTowerID(inputValue) {
    if(!inputValue || inputValue.length < 3 ) {
      ...
      const getTowerID = await getDatafromAPITSEL('/site_op?where={"site_id":{"$regex":"'+inputValue+'", "$options":"i"}}');
  ...
  async submitPSWH(){
    const res = await patchDatatoAPINODE('/plantspec/submitPlantspec/'+this.props.match.params.id, {}, this.state.tokenUser)
    if(res !== undefined && res.data !== undefined){
  ...
  async updateMaterialQty(){
    ...
    const dataPackage = dataPS.packages;
    const dataQtyNew = this.state.qty_material_new;
    for(let i = 0; i < dataPackage.length; i++){
      for(let j = 0; j < dataPackage[i].materials.length; j++){
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Card>
          <CardHeader>
            <span style={{lineHeight :'2', fontSize : '17px'}}>PS Warehouse </span>
            {this.state.ps_wh_data.submission_status !== "SUBMITTED" && (
              <Fragment>
                <Button size="sm" onClick={this.submitPSWH} color='primary' style={{float : 'right', margin : '0 10px 0 10px'}}>
                  Submit
                </Button>
          ...
          <CardBody>
            <div>
              <Row>
                <Col md="12">
                  <table style={{width : '100%', marginBottom : '0px', fontSize : '20px', fontWeight : '500'}}>
                    <tbody>
                      <tr>
                        <td colSpan="4" style={{textAlign : 'center', color : 'rgba(59,134,134,1)', fontSize : '21px'}}>Plant Spec Warehouse Detail</td>
                      </tr>
                      <Fragment>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Plant Spec WH ID : {this.state.ps_wh_data.no_plantspec}</td>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Project Name : {this.state.ps_wh_data.project_name}</td>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Origin Warehouse : {this.state.ps_wh_data.origin_wh !== undefined ? this.state.ps_wh_data.origin_wh.value : null}</td>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Destination Warehouse : {this.state.ps_wh_data.destination_wh !== undefined ? this.state.ps_wh_data.destination_wh.value : null}</td>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>MR Warehouse Related : {this.state.ps_wh_data.mra_id}</td>
                      </tr>
                      <tr>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Type PS : {this.state.ps_wh_data.plantspec_type}</td>
                      </tr>
                      <tr>
                        <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Current Status : {this.state.ps_wh_data.current_status}</td>
                      </tr>
                      </Fragment>
                    </tbody>
                  </table>
              ...
              {/* }<Row>
                <Col md="12">
                  {this.state.edit_qty_mat_WH === true && (
