...
import {apiSendEmail, postDatatoAPINODE, getDatafromAPITSEL, getDatafromAPINODE, patchDatatoAPINODE} from '../../helper/asyncFunction';
...
let headerFormatDSAMigration = ["id", "activity_id", "project", "category", "eta", "etd", "deliver_by", "dismantle_by", "wh_destination", "activity_id_destination", "project_destination", "assignment_id", "mra_type", "mr_related", "note"];
...
class MRDismantleBulk extends Component {
  ...
  getASPList() {
    getDatafromAPITSEL('/vendor_data_non_page?where={"Type":"ASP"}').then(res => {
  ...
  async getProjectinBulk(array_project){
    ...
    const dataProject = await getDatafromAPITSEL('/project_sorted_non_page?where={"Project" : {"$in" : ['+arrayProject+']}}');
  ...
  async saveDataMRABulk(){
    ...
    const respondSaveDSA = await postDatatoAPINODE('/matreq-srn/createMatreqDismantle', dataUploadMigration, this.state.tokenUser);
    if(respondSaveDSA.data !== undefined && respondSaveDSA.status >= 200 && respondSaveDSA.status <= 300 ) {
      this.setState({ action_status : 'success' });
    } else{
      if (respondSaveDSA.response !== undefined && respondSaveDSA.response.data !== undefined && respondSaveDSA.response.data.error !== undefined) {
  ...
  exportFormatBulkMR = async () =>{
    ...
    let headerRow = ["id", "activity_id", "project", "category", "eta", "etd", "deliver_by", "dismantle_by", "wh_destination", "activity_id_destination", "project_destination", "assignment_id", "mra_type", "mr_related", "note"];
    ws.addRow(headerRow);
    let exRow1 = ["new", null, null, "TWH", "2020-10-02", "2020-10-06", "DSP", null, "JKT1", null, null, null, 1, null, null];
    ws.addRow(exRow1);
          ...
          <CardBody className='card-UploadBoq'>
            <Row>
              <Col>
                ...
                <Button color="success" onClick={this.saveDataMRABulk} style={{float : 'right'}} disabled={this.state.action_status === "failed" || this.state.rowsXLS.length === 0}>
            ...
            <table style={{width : '100%', marginBottom : '0px', fontSize : '20px', fontWeight : '500'}}>
              <tbody>
                <tr>
                  <td colSpan="4" style={{textAlign : 'center'}}>MRA UPLOADER PREVIEW</td>
                </tr>
                <tr>
