...
class PSDisList extends Component {
  ...
  async getAllPSDis() {
    ...
    let res = await this.getDataFromAPINODE(
      "/plantspec-srn?srt=_id:-1&noPg=1&q=" + whereAnd
    );
    if (res.data !== undefined) {
      const items = res.data.data;
  ...
  async downloadPSDisList() {
    ...
    for (let i = 0; i < allPSA.length; i++) {
      ws.addRow([
        allPSA[i].no_plantspec_srn,
        allPSA[i].plantspec_srn_category,
        allPSA[i].project_name,
        allPSA[i].site_info.map((si) => si.site_id).join(", "),
        allPSA[i].site_info.map((si) => si.site_name).join(", "),
        allPSA[i].site_info.map((si) => si.region).join(", "),
        allPSA[i].current_status,
        new Date(allPSA[i].created_on),
      ]);
    }
    this.toggleLoading();
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownItem onClick={this.downloadPSDisList}>
                            ...
                            <i
                              ...
                              aria-hidden="true"
                            ></i>
                            Download PS SRN List
                          </DropdownItem>
                          <DropdownItem onClick={this.downloadPSDisListFM}>
                            {" "}
                            <i
                              className="fa fa-file-text-o"
                              aria-hidden="true"
                            ></i>
                            Download PS SRN List For Migration
                          </DropdownItem>
                        </DropdownMenu>
                      </Dropdown>
                    </React.Fragment>
                  )}
                <Link to={"/srn/ps-srn-creation"}>
                  <Button
                    color="success"
                ...
                <Table responsive striped bordered size="sm">
                  ...
                  <tbody>
                    ...
                    {this.state.ps_dis_list.map((list, i) => (
                      <tr key={list._id}>
                        <td>
                          <Link to={"/srn/ps-srn-detail/" + list._id}>
                            {list.no_plantspec_srn}
                          </Link>
                        </td>
                        <td>{list.plantspec_srn_category}</td>
                        <td>{list.project_name}</td>
                        <td>
                          {list.site_info.map((si) => si.site_id).join(", ")}
                        </td>
                        <td>
                          {list.site_info.map((si) => si.site_name).join(", ")}
                        </td>
                        <td>{list.current_status}</td>
                        <td>{list.mra_id}</td>
                        <td>
                          {list.site_info.map((si) => si.region).join(", ")}
                        </td>
