...
import { getDatafromAPITSEL, postDatatoAPINODE, getDatafromAPINODE} from "../../helper/asyncFunction";
...
class MRDisCreation extends Component {
  ...
  async loadOptionsWPID(inputValue) {
    if(!inputValue || inputValue.length < 3 ) {
      ...
      const getTowerID = await getDatafromAPITSEL('/custdel_op?where={"WP_ID":{"$regex":"'+inputValue+'", "$options":"i"}}&projection={"WP_ID":1,"WP_Name":1,"CD_Info_Project_Name":1,"CD_Info_Project":1,"Site_Info_SiteID_Value_NE":1,"Site_Info_SiteName_NE":1}');
