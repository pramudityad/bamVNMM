...
class DSADCreation extends Component {
  ...
  async loadOptionsMR(inputValue) {
    if (!inputValue) {
      ...
      let mr_list = [];
      const getMR = await this.getDataFromAPINODE('/matreq-srn?srt=_id:-1&q={"mra_id":{"$regex":"' + inputValue + '", "$options":"i"}}&v={"mra_id":1, "dsa_number":1}');
      if (getMR !== undefined && getMR.data !== undefined) {
        this.setState({ list_mr_selection: getMR.data.data }, () =>
          getMR.data.data.map(mr =>
            mr_list.push({ 'label': mr.mra_id !== undefined ? mr.mra_id : null, 'value': mr._id, 'dsa_number' : mr.dsa_number })))
      }
      return mr_list;
    }
  }
  ...
  handleChangeMR = async (newValue) => {
    this.toggleLoading();
    const getMatreqDSA = await this.getDataFromAPINODE('/matreq-srn-dsa/'+newValue.value);
    ...
    const getNN = await this.getDataFromAPITSEL('/custdel_sorted_non_page?where={"WP_ID":"' + dataMR.cust_del[0].cd_id + '"}');
  ...
  async loadOptionsDSA(inputValue) {
    if (!inputValue) {
      ...
      let dsa_list = [];
      const getDSA = await this.getDataFromAPITSEL('/dsa_price_sorted?where={"dsa_price_id":{"$regex":"' + inputValue + '", "$options":"i"}, "vendor_code_actual" : "'+this.state.list_mr_selected.dsp_company_code+'"}');
    ...
    if(this.state.list_mr_selected.po_dsa_list !== undefined && this.state.list_mr_selected.po_dsa_list !== null && this.state.list_mr_selected.po_dsa_list.length !== 0){
      ...
      if(updateDSA.primary_section.length === 0){
        ...
        if(updateDSA.primary_section[0].sub_category !== null && updateDSA.primary_section[0].service_master === null){
          ...
          let res = await this.patchDatatoAPINODE('/matreq/dsaCreation/' + _id, { "account_id": "2", "mrCategory" : "Return", "submitType" : 0, "data": updateDSA });
                    ...
                    <Row>
                      ...
                      <Col md="3">
                        <FormGroup style={{ paddingLeft: "16px" }}>
                          ...
                          <Input readOnly value={this.state.list_mr_selected !== null ? this.state.list_mr_selected.current_mra_status : ""}/>
                        </FormGroup>
                      </Col>
                    </Row>
                    ...
                    <Row>
                      <Col md="3">
                        <FormGroup style={{ paddingLeft: "16px" }}>
                          ...
                          <Input type="text" name="dsa_number" readOnly value={this.state.list_mr_selected !== null ? "DSA"+this.state.list_mr_selected.mra_id : ""} />
                        </FormGroup>
                        <FormGroup style={{ paddingLeft: "16px" }}>
                          <Label>Project</Label>
