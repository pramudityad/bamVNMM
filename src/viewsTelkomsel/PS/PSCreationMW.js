...
class PSCreationMW extends Component {
  ...
  getDataTech() {
    this.getDataFromAPINODEWithParams('/plantspec/tssrBom/'+this.state.list_site_tech_boq_selected+'/linkId/'+this.state.list_site_selected).then( resTech => {
  ...
  async saveTssrBOM(){
    ...
    const respondSaveTSSR = await this.postDatatoAPINODE('/plantspec/createPlantSpec2', {"hos_list": dataHos, "psData" : this.state.data_tssr_selected_origin, psType : "MW"});
  ...
  async previewTSSRtoPS(){
    ...
    this.postDatatoAPINODE('/plantspec/getTssrData2', {"data" : dataTSSRforGet, "itemPackage": true}).then(res => {
      if(res.data !== undefined){
        this.getHosLibrary();
        this.setState({data_tssr_selected_origin : JSON.parse(JSON.stringify(res.data.psData))})
  ...
  getHosPerPP(id_tssr_boq_site_doc, no_tssr_boq_site, id_pp_doc, pp_id, type, qty_ne, qty_fe){
    ...
    if(findPP !== undefined && findPP.material_hos !== undefined){
      ...
      return(
        <Input type="select" name={id_tssr_boq_site_doc+" /// "+no_tssr_boq_site+" /// "+id_pp_doc+" /// "+pp_id+" /// "+type+" /// "+qty_ne+" /// "+qty_fe} onChange={this.handleChangeVariant} value={valueSelect !== undefined ? valueSelect.hos_name : null}>
  ...
  getHosPerPPFE(id_tssr_boq_site_doc, no_tssr_boq_site, id_pp_doc, pp_id, type, qty_ne, qty_fe){
    const dataHos = this.state.list_hos;
    const findPP = dataHos.find(dh => dh.pp_id === pp_id);
    if(findPP !== undefined && findPP.material_hos !== undefined){
      const allHosPP = findPP.material_hos.map(value => value.hos_list.map(child => child)).reduce((l, n) => l.concat(n), []);
      ...
      const valueSelect = this.state.hos_selected.find(hs => hs.id_tssr_boq_site_doc === id_tssr_boq_site_doc && hs.pp_id === pp_id);
      return(
        <Input type="select" name={id_tssr_boq_site_doc+" /// "+no_tssr_boq_site+" /// "+id_pp_doc+" /// "+pp_id+" /// "+type+" /// "+qty_ne+" /// "+qty_fe} onChange={this.handleChangeVariant} value={valueSelect !== undefined ? valueSelect.hos_name_fe : null}>
          <option value={null}></option>
          {allHosPPUniq.map(mh =>
            <option value={mh}>{mh}</option>
          )}
  ...
  handleChangeQtyMW(e, id_tssr_boq_site_doc, no_tssr_boq_site, id_pp_doc, pp_id, type){
    const value = e.target.value;
    let dataHosSelected = this.state.hos_selected;
    let findIdxSelected = dataHosSelected.findIndex(dhs => dhs.pp_id === pp_id && dhs.id_tssr_boq_site_doc === id_tssr_boq_site_doc);
    if(findIdxSelected !== -1){
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Card>
          <CardHeader>
            <span style={{lineHeight :'2', fontSize : '17px'}} >Plant Spec Transmission Creation</span>
          </CardHeader>
          <CardBody className='card-UploadBoq'>
            ...
            <table style={{width : '100%', marginBottom : '0px', fontSize : '20px', fontWeight : '500'}}>
              <tbody>
                <tr>
                  <td colSpan="4" style={{textAlign : 'center'}}>PLANT SPEC TRANSMISSION PREVIEW</td>
                </tr>
                <tr>
                  <td colSpan="4" style={{textAlign : 'center', fontSize : '12.5px'}}>WP ID : {this.state.data_tssr_selected.map(dts => dts.wp_id)}</td>
                </tr>
              </tbody>
            </table>
              ...
              <Table hover bordered striped responsive size="sm">
                <thead style={{backgroundColor : '#0B486B', color : 'white'}}>
                  <tr>
                    ...
                    <th colSpan="2" className="fixedhead" style={{verticalAlign : 'middle'}}>Variant</th>
                  ...
                  <tr>
                    <th className="fixedhead" style={{width : '100px', verticalAlign : 'middle'}}>Var NE</th>
                    <th className="fixedhead" style={{width : '100px', verticalAlign : 'middle'}}>Var FE</th>
                    <th className="fixedhead" style={{width : '100px', verticalAlign : 'middle'}}>Qty PS NE</th>
                  ...
                  {this.state.data_tssr_selected.map( tssr =>
                      tssr.listOfPackage.map( pp =>
                        ...
                        <tr style={{backgroundColor : '#E5FCC2'}} className="fixbody">
                          ...
                          <td>{this.getHosPerPP( pp.id_tssr_boq_site_doc, pp.no_tssr_boq_site, pp.id_pp_doc, pp.pp_id, "ne", pp.qty, pp.qty_fe)}</td>
                          <td>{this.getHosPerPPFE( pp.id_tssr_boq_site_doc, pp.no_tssr_boq_site, pp.id_pp_doc, pp.pp_id, "fe", pp.qty, pp.qty_fe)}</td>
                          <td>{pp.uom}</td>
                          <td align='center'>
                            <Input type="number" min="0" defaultValue={(pp.qty).toFixed(2)} onChange={e => {this.handleChangeQtyMW(e, pp.id_tssr_boq_site_doc, pp.no_tssr_boq_site, pp.id_pp_doc, pp.pp_id, "ne")} } />
                          </td>
                          <td align='center'>
                            <Input type="number" min="0" defaultValue={(pp.qty).toFixed(2)} onChange={e => {this.handleChangeQtyMW(e, pp.id_tssr_boq_site_doc, pp.no_tssr_boq_site, pp.id_pp_doc, pp.pp_id, "fe")} } />
                          </td>
                        </tr>
                        {pp.list_material.map(material =>
                          <tr style={{backgroundColor : 'rgba(248,246,223, 0.5)'}} className="fixbody">
                            ...
                            <td align='center'>{pp.qty_fe*material.qty}</td>
                          </tr>
                        )}
                        </Fragment>
                      )
