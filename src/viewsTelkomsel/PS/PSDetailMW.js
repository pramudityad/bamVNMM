...
class PSDetailMW extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
        ...
        qty_ps : [["bam_id","bundle_id","bundle_name","program","material_id_plan","material_name_plan","material_id_actual","material_name_actual","uom","qty"]],
    ...
    this.getDataFromAPINODE('/plantspec/'+_id_tssr).then( res => {
      if(res.data !== undefined){
        this.setState({ tssrData : res.data.data }, () => {
          ...
          this.prepareView(res.data.data);
  ...
  prepareView(){
    ...
    let dataPS = Object.assign({}, this.state.tssrData);
    ...
    for(let i = 0; i < dataPPNE.length; i++){
      ...
      for(let j = 0; j < materialConcatUniq.length; j++){
        ...
        // materialConcatUniq[j].push(materialConcatIdx);
      }
  ...
  handleChangeQTY(e, i, u){
    ...
    let dataQtyChange = this.state.qty_change;
    const Data_package = Data_tssr.packages.find(dp => dp._id === i);
    if(Data_package !== undefined){
      let Data_mat =  Data_package.materials.find(dpm => dpm._id === u);
      if(Data_mat !== undefined){
        const findMatQty = dataQtyChange.findIndex(dpmc => dpmc._id === u);
        if(findMatQty !== -1){
          dataQtyChange[findMatQty]["qty"] = value_qty;
        }else{
          Data_mat["product_name"] = Data_package.product_name;
          Data_mat["program"] = Data_package.program;
          Data_mat["qty"] = value_qty;
          dataQtyChange.push(Data_mat);
        }
      }
    }
    this.setState({qty_change : dataQtyChange});
  }
  ...
  async downloadMaterialTSSRUpload() {
    ...
    let headerRow = ["bam_id", "bundle_id", "bundle_name", "program", "site_title", "material_id_plan", "material_name_plan", "material_id_actual", "material_name_actual", "uom", "qty", "stock_warehouse", "inbound_warehouse", "availability", "source_material"];
    ws.addRow(headerRow);
    ...
    for(let i = 0; i < dataItemTSSR.length; i++){
      for(let j = 0; j < dataItemTSSR[i].materials.length; j++){
        ...
        ws.addRow([dataMatIdx._id, dataItemTSSR[i].pp_id, dataItemTSSR[i].product_name, dataItemTSSR[i].program, dataMatIdx.site_title, dataMatIdx.material_id_plan, dataMatIdx.material_name_plan, dataMatIdx.material_id, dataMatIdx.material_name, dataMatIdx.uom, dataMatIdx.qty, qty_wh, qty_inbound, (dataMatIdx.qty) < qty_wh ? "OK":"NOK"]);
      }
    }
  ...
  async downloadMaterialTSSRUploadNOK() {
    ...
    let headerRow = ["bam_id", "tssr_id", "bundle_id", "bundle_name", "program", "site_title", "material_id_plan", "material_name_plan", "material_id_actual", "material_name_actual", "uom", "qty", "stock_warehouse", "inbound_warehouse", "availability", "source_material"];
    ws.addRow(headerRow);
    let list_material_id = [];
    for(let i = 0; i < dataItemTSSR.length; i++){
      for(let j = 0; j < dataItemTSSR[i].materials.length; j++){
  ...
  async saveUpdateMaterialWeb(){
    this.toggleLoading();
    const qtyWeb = this.state.qty_change;
    let dataWeb = [["bam_id","bundle_id","bundle_name","program","site_title","material_id_plan","material_name_plan","material_id_actual","material_name_actual","uom","qty"]]

    for(let j = 0; j < qtyWeb.length; j++){
      dataWeb.push(
        [qtyWeb[j]._id, qtyWeb[j].pp_id, qtyWeb[j].product_name, qtyWeb[j].program, qtyWeb[j].site_title, qtyWeb[j].material_id_plan, qtyWeb[j].material_name_plan, qtyWeb[j].material_id_plan, qtyWeb[j].material_name_plan, qtyWeb[j].uom, qtyWeb[j].qty]
      )
    }
    const dataTSSR = this.state.tssrData;
    ...
    return (
      <div>
        ...
        <Row>
          ...
          <Card>
            <CardHeader>
              <span style={{lineHeight :'2', fontSize : '15px'}} >Detail Plant Spec Transmission</span>
            ...
            {this.state.data_tssr !== null ?
              ...
              <table style={{width : '100%', marginBottom : '0px', fontSize : '20px', fontWeight : '500'}}>
                <tbody>
                  <tr>
                    <td colSpan="4" style={{textAlign : 'center', color : 'rgba(59,134,134,1)', fontSize : '21px'}}>Plant Spec Transmission DETAIL</td>
                  </tr>
                  {this.state.tssrData !== null && (
                    ...
                    <tr>
                      <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Site ID NE : {this.state.tssrData.site_info[0].site_id}</td>
                    </tr>
                    <tr>
                      <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>Site ID FE : {this.state.tssrData.site_info[1].site_id}</td>
                    </tr>
                    <tr>
                      <td colSpan="4" style={{fontSize : '15px', textAlign : 'center', color : 'rgba(59,134,134,1)'}}>WP ID : {this.state.tssrData.site_info[0].cd_id}</td>
                    </tr>
                    <tr>
                  ...
                  <Table hover bordered striped responsive size="sm">
                    <thead style={{backgroundColor : '#0B486B', color : 'white'}}>
                      <tr>
                        ...
                        <th colSpan="4" className="fixedhead" style={{width : '100px', verticalAlign : 'middle'}}>Total Qty per PP</th>
                      ...
                      <tr>
                        ...
                        <th className="fixedhead" style={{width : '100px', verticalAlign : 'middle'}}>Qty PS FE</th>
                    ...
                    <tbody>
                      {this.state.tssrData !== null && Array.isArray(this.state.tssrData.packages) && (
                        this.state.product_package_ps_mw.filter(dp => dp.product_type.toLowerCase() !== "svc" && dp.product_type.toLowerCase() !== "hwac" && dp.product_type.toLowerCase() !== "sw").map((pp, idxPP) =>
                          <Fragment>
                            <tr style={{backgroundColor : '#E5FCC2'}} className="fixbody">
                              ...
                              <td align='center'>{pp.qty_fe.toFixed(2)}</td>
                              <td align='center'></td>
                              <td align='center'></td>
                              <td align='center'></td>
                            </tr>
                            {pp.materials.map((material,idxMat) =>
                              <tr style={{backgroundColor : 'rgba(248,246,223, 0.5)'}} className="fixbody">
                                ...
                                <td>{material.uom}</td>
                                <td key={idxMat} align='center'>
                                  {this.state.collapseUpload === true ? (
                                    <Input type="number" min="0" disabled={material._id_ne === undefined} defaultValue={(material.qty).toFixed(2)} onChange={e => {this.handleChangeQTY(e, pp._id_ne, material._id_ne)} } />
                                  ) : material.qty }
                                </td>
                                <td key={idxMat} align='center'>
                                  {this.state.collapseUpload === true ? (
                                    <Input type="number" min="0" disabled={material._id_fe === undefined} defaultValue={(material.qty_fe).toFixed(2)} onChange={e => {this.handleChangeQTY(e, pp._id_fe, material._id_fe)} } />
                                  ) : material.qty_fe }
                                </td>
            ...
            <CardFooter>
              {(this.state.tssrData !== null && this.state.tssrData.mr_id === null && (this.state.userRole.includes('BAM-Engineering') === true || this.state.userRole.includes('Admin') === true )) && (
...
export default connect(mapStateToProps)(PSDetailMW);
