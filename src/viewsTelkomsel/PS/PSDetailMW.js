...
class PSDetailMW extends Component {
  ...
  prepareView() {
    ...
    for (let i = 0; i < dataPPNE.length; i++) {
      ...
      const dataFEIdx = dataPPPS.find(
        (fe) => fe.site_title === "FE" && fe.id_pp_doc === dataPPNE[i].id_pp_doc
      );
  ...
  uniqObjectArray(array, field) {
    ...
    array.map(
      (arr) =>
        !arrUniq.find((au) => au[field] === arr[field]) &&
        arrUniq.push(Object.assign(arr))
    );
    return arrUniq;
  }
  ...
  async saveUpdateMaterialWeb() {
    ...
    for (let j = 0; j < qtyWeb.length; j++) {
      dataWeb.push([
        qtyWeb[j]._id,
        qtyWeb[j].pp_id,
        qtyWeb[j].product_name,
        qtyWeb[j].program,
        qtyWeb[j].site_title,
        qtyWeb[j].material_id_plan,
        qtyWeb[j].material_name_plan,
        qtyWeb[j].material_id_plan,
        qtyWeb[j].material_name_plan,
        qtyWeb[j].uom,
        qtyWeb[j].qty,
  ...
  async saveAdditional() {
    ...
    for (let i = 0; i < addMaterial.length; i++) {
      dataUpload.push([
        ...
        parseFloat(addMaterial[i].qty_fe),
  ...
  AdditionalForm() {
    ...
    const addPP = this.state.tssrData.packages.filter(
      (e) => e.pp_id === "AdditionalMaterial" && e.site_title === "NE"
    );
    const addMaterial = addPP
      .map((value) => value.materials.map((child) => child))
      .reduce((l, n) => l.concat(n), []);
    const addPPFE = this.state.tssrData.packages.filter(
      (e) => e.pp_id === "AdditionalMaterial" && e.site_title === "FE"
    );
    const addMaterialFE = addPPFE
      .map((value) => value.materials.map((child) => child))
      .reduce((l, n) => l.concat(n), []);
    for (let i = 0; i < addMaterial.length; i++) {
      let qty_fe = 0;
      let qty_ne = 0;
      let matFE = addMaterialFE.find(
        (af) => af.material_id === addMaterial[i].material_id
      );
      if (matFE !== undefined) {
        qty_fe = matFE.qty;
      }
            ...
            <Card>
              <CardHeader>
                <span style={{ lineHeight: "2", fontSize: "15px" }}>
                  Detail Plant Spec Transmission
                </span>
                  ...
                  <Fragment>
                    ...
                    >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownItem
                          onClick={this.exportMaterialPSReportBundling}
                        >
                          {" "}
                          <i
                        ...
                        <DropdownItem
                          onClick={this.downloadMaterialTSSRUploadNOK}
                        >
                          {" "}
                          <i
                            className="fa fa-file-text-o"
                ...
                >
                  <tbody>
                    <tr>
                      ...
                      >
                        Plant Spec Transmission DETAIL
                      </td>
                    </tr>
                    {this.state.tssrData !== null && (
                      <Fragment>
                        ...
                        <tr>
                          ...
                          >
                            Site ID FE :{" "}
                            {this.state.tssrData.site_info[1].site_id}
                          </td>
                        </tr>
                        <tr>
                          <td
                          ...
                          >
                            WP ID : {this.state.tssrData.site_info[0].cd_id}
                          </td>
                        </tr>
                        <tr>
                          <td
                    ...
                    <Table hover bordered striped responsive size="sm">
                      ...
                      <tbody>
                        {this.state.tssrData !== null &&
                          ...
                          this.state.product_package_ps_mw
                            .filter(
                              (dp) =>
                                dp.product_type.toLowerCase() !== "svc" &&
                                dp.product_type.toLowerCase() !== "hwac" &&
                                    ...
                                    <td key={idxMat} align="center">
                                      {this.state.collapseUpload === true ? (
                                        <Input
                                          ...
                                          min="0"
                                          disabled={
                                            material._id_fe === undefined
                                          }
                                          defaultValue={material.qty_fe.toFixed(
                                            2
                                          )}
                                          onChange={(e) => {
                                            this.handleChangeQTY(
