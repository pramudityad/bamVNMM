...
const arrayFilter = ["no_plantspec", "project_name", "site_info.site_id", "site_info.site_name", "site_info.cd_id", "current_status", "mr_id"];
...
class PSList extends Component {
  ...
  getTssrList(){
    ...
    (this.state.filter_list["site_info.cd_id"] !== null && this.state.filter_list["site_info.cd_id"] !== undefined) && (filter_array.push('"site_info.cd_id":{"$regex" : "' + this.state.filter_list["site_info.cd_id"] + '", "$options" : "i"}'));
                  ...
                  <tbody>
                    {this.state.tssr_list.map((list, i) =>
                      <tr key={list._id}>
                        ...
                        <td>{list.site_info.map(si => si.cd_id).join(", ")}</td>
