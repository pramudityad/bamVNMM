...
import {getDatafromAPINODE, postDatatoAPINODE, patchDatatoAPINODE, getDatafromAPITSEL} from '../../../helper/asyncFunction';
...
class TechnicalBoq extends Component {
  ...
  getTechBoqData(_id_tech){
    getDatafromAPINODE('/techBoq/'+_id_tech+'?package=1', this.state.tokenUser).then(res => {
  ...
  async updateTechBoq(e){
    ...
    let patchTech = await patchDatatoAPINODE('/techBoq/updateTechBoqData/'+this.state.data_tech_boq._id, dataPatch, this.state.tokenUser);
  ...
  getVersionTechBoqData(_id_tech, ver){
    getDatafromAPINODE('/techBoq/'+_id_tech+'/ver/'+ver+'?package=1', this.state.tokenUser).then(res => {
      if(res.data !== undefined){
        const dataTech = res.data;
        if(res.data.data !== undefined){
  ...
  async approvalTechnical(e){
    ...
    let patchData = await patchDatatoAPINODE('/techBoq/approval/'+this.state.data_tech_boq._id, {"operation":currValue})
  ...
  async checkingFormatTech(rowsTech){
    ...
    let postCheck = await postDatatoAPINODE(urlCheck, dataCheck, this.state.tokenUser);
  ...
  saveTechBoq = async () => {
    ...
    let postTech = await postDatatoAPINODE('/techBoq/createTechBoqData', dataPost, this.state.tokenUser);
 ...
 getListVersion(_id){
  ...
  async submitTSSR(e){
    ...
    let patchData = await postDatatoAPINODE('/tssr/createTssr/'+this.state.data_tech_boq._id, {"itemPackage" : true})
    if(patchData.data !== undefined){
      this.setState({action_status : 'success'});
    }else{
      if(patchData.response !== undefined){
