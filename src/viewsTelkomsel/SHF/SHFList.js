...
class SHFList extends Component {
  ...
  async downloafSHFFile(e) {
    ...
    if (
      data_shf !== undefined &&
      data_shf.shf_images !== undefined &&
      data_shf.shf_images !== null &&
      data_shf.shf_images.length !== 0
    ) {
      const data_shf_file = data_shf.shf_images.find((si) => si._id === idFile);
      const resFile = await getDatafromAPINODEFile(
        "/siteHandlingFee/downloadShfDoc/" + id + "/file/" + idFile,
        this.props.dataLogin.token,
        data_shf_file.mime_type
      );
      if (resFile !== undefined) {
        saveAs(
          new Blob([resFile.data], { type: data_shf_file.mime_type }),
          data_shf_file.file_name
        );
      }
    }
  }
                    ...
                    <React.Fragment>
                      ...
                      >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownItem onClick={this.downloadSHFList}>
                            {" "}
                            <i
                  ...
                  <tbody>
                    ...
                    {this.state.shf_list.map((list, i) => (
                      <React.Fragment>
                        <tr
                          key={list._id}
                          style={{ backgroundColor: "rgba(31, 111, 139, 0.2)" }}
                        >
                          <td>{list.shf_no}</td>
