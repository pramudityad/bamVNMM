...
const array_field = ["shf_no", "related_to", "site_id", "site_name", "site_location.latitude", "site_location.longitude", "distance", "amount_paid", "receiver_name", "receiver_email", "receiver_note",  "created_by", "created_on"]
...
class SHFList extends Component {
  ...
  async downloadAllMRMigration() {
    ...
    const allocexport = await wb.xlsx.writeBuffer();
    saveAs(new Blob([allocexport]), 'SHF List For Status Migration from SH Template.xlsx');
  }
                ...
                {((this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-Mover") === -1)) && (
                  ...
                  <Dropdown size="sm" isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{ float: 'right', marginRight : '10px' }}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownItem onClick={this.downloadPSDislist}> <i className="fa fa-file-text-o" aria-hidden="true"></i>Download SHF List</DropdownItem>
                    </DropdownMenu>
                  </Dropdown>
                </React.Fragment>
                )}
