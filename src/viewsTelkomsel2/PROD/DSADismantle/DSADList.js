...
class DSAList extends Component {
  ...
  async getDSAListAll() {
    ...
    let res = await this.getDataFromAPINODE(
      "/matreq-srn?srt=_id:-1&noPg=1&q=" + whereAnd
    );
    if (res.data !== undefined) {
      const items = res.data.data;
  ...
  async downloadDSAlist() {
    ...
    let dsaStatus = [
      "Process - Sent MR Request Name (Rule)",
      "Process - Sent MR Request Time (Rule)",
      "Process - MR Receipt (DC) Name (Rule)",
      "Process - MR Receipt (DC) Time (Rule)",
    ];
    let dsaDSAGeneral = [
      "MOT",
      "DSP",
      "MOS Date",
      "DSA Actual",
      "Cost MR Transportation ",
      "Cost MR Additional",
      "DSA Created Date",
      "DSA Created By",
      "LDM  Approval Name",
      "LDM  Approval Date",
      "LDM Admin Approval Name",
      "LDM Admin Approval Date",
      "GR By",
      "GR Date",
      "GR Number",
    ];
    ...
    for (let i = 0; i < btsSec2; i++) {
      dataSec2.push(
        "DSA - Additional Category " + (i + 1),
        "DSA - Additional Service Code " + (i + 1),
        "DSA - Additional Service Master " + (i + 1),
        "DSA - Additional Service Price " + (i + 1),
        "DSA - Additional Service Quantity " + (i + 1),
        "DSA - Additional Service Total Price " + (i + 1),
        "DSA - Additional Service Short Text " + (i + 1),
        "DSA - Additional Service Long Text " + (i + 1)
      );
    }
    for (let i = 0; i < btsSec3; i++) {
      dataSec2.push(
        "DSA - Section 3 Description " + (i + 1),
        "DSA - Section 3 Type of Cost " + (i + 1),
        "DSA - Section 3 Price " + (i + 1)
      );
    }
    ...
    for (let i = 0; i < allMR.length; i++) {
      ...
      if (
        allMR[i].site_info !== undefined &&
        allMR[i].site_info[1] !== undefined
      ) {
        site_info_2 = allMR[i].site_info[1];
      }
      let approved_ldm = {};
      let lastIndexApproved = allMR[i].mr_status.filter(
        (lio) =>
          lio.mr_status_name === "MATERIAL_REQUEST" &&
          lio.mr_status_value === "APPROVED"
      );
      if (lastIndexApproved.length !== 0) {
        approved_ldm = allMR[i].mr_status[lastIndexApproved.length - 1];
      }
      let mr_mos = {};
      let lastIndexMOS = allMR[i].mr_status.filter(
        (lio) =>
          lio.mr_status_name === "MATERIAL_REQUEST" &&
          lio.mr_status_value === "ON SITE"
      );
      if (lastIndexMOS.length !== 0) {
        mr_mos = lastIndexMOS[lastIndexMOS.length - 1];
      }
      let dsa_creation = {};
      let lastIndexDSAC = allMR[i].dsa_status.filter(
        (lio) =>
          lio.dsa_status_name === "DSA" && lio.dsa_status_value === "CREATED"
      );
      if (lastIndexDSAC.length !== 0) {
        dsa_creation = lastIndexDSAC[lastIndexDSAC.length - 1];
      }
      let dsa_ldma = {};
      let lastIndexLDMA = allMR[i].dsa_status.filter(
        (lio) =>
          lio.dsa_status_name === "DSA_LDM_APPROVAL" &&
          lio.dsa_status_value === "APPROVED"
      );
      if (lastIndexLDMA.length !== 0) {
        dsa_ldma = lastIndexLDMA[lastIndexLDMA.length - 1];
      }
      let dsa_ldmaa = {};
      let lastIndexLDMAA = allMR[i].dsa_status.filter(
        (lio) =>
          lio.dsa_status_name === "DSA_LDM_ADMIN_APPROVAL" &&
          lio.dsa_status_value === "APPROVED"
      );
      ...
      let dsa_mr_row = [
        ...
        site_info_1.site_id,
        site_info_1.site_name,
        site_info_2.site_id,
        site_info_2.site_name,
        allMR[i].current_mr_status,
        allMR[i].origin !== undefined ? allMR[i].origin.value : null,
        ...
        lcc_data.no_lcc,
        lcc_data.budget,
        lcc_data.no_lcc !== undefined ? new Date(lcc_data.no_lcc) : null,
        allMR[i].no_po_dsa,
      ];
      let dsa_mr_row_status = [
        allMR[i].creator !== undefined ? allMR[i].creator.email : null,
        new Date(allMR[i].created_on),
        approved_ldm.mr_status_updater,
        approved_ldm.mr_status_date !== undefined
          ? new Date(approved_ldm.mr_status_date)
          : null,
      ];
      let dsa_mr_row_das_general = [
        "MOT",
        allMR[i].dsp_company,
        mr_mos.mr_status_date !== undefined
          ? new Date(mr_mos.mr_status_date)
          : null,
        allMR[i].dsa_total_value,
        allMR[i].primary_section
          .filter(
            (ps) => ps.total_price !== undefined && ps.total_price !== null
          )
          .reduce((a, b) => a + parseFloat(b.total_price), 0),
        allMR[i].second_section.service_details
          .filter(
            (ps) => ps.total_price !== undefined && ps.total_price !== null
          )
          .reduce((a, b) => a + parseFloat(b.total_price), 0) +
          allMR[i].third_section.service_details
            .filter((ps) => ps.price !== undefined && ps.price !== null)
            .reduce((a, b) => a + parseFloat(b.price), 0),
        dsa_creation.dsa_status_updater,
        dsa_creation.dsa_status_date !== undefined
          ? new Date(dsa_creation.dsa_status_date)
          : null,
        dsa_ldma.dsa_status_updater,
        dsa_ldma.dsa_status_date !== undefined
          ? new Date(dsa_ldma.dsa_status_date)
          : null,
        dsa_ldmaa.dsa_status_updater,
        dsa_ldmaa.dsa_status_date !== undefined
          ? new Date(dsa_ldmaa.dsa_status_date)
          : null,
        allMR[i].dsa_gr_by,
        allMR[i].dsa_gr_date,
        allMR[i].dsa_gr_no,
      ...
      if (allMR[i].primary_section !== undefined) {
        ...
        for (let j = 0; j < btsSec1; j++) {
          ...
          if (dataPrimaryIdx !== undefined) {
            rowSec1.push(
              dataPrimaryIdx.sub_category,
              dataPrimaryIdx.service_master,
              dataPrimaryIdx.price,
              dataPrimaryIdx.qty,
              dataPrimaryIdx.total_price,
              dataPrimaryIdx.long_text
            );
          } else {
      ...
      if (
        allMR[i].second_section !== undefined &&
        allMR[i].second_section.service_details !== undefined
      ) {
        for (let j = 0; j < btsSec2; j++) {
          let dataSecondaryIdx = allMR[i].second_section.service_details[j];
          if (dataSecondaryIdx !== undefined) {
            rowSec2.push(
              dataSecondaryIdx.sub_category,
              dataSecondaryIdx.service_master,
              dataSecondaryIdx.service_master,
              dataSecondaryIdx.price,
              dataSecondaryIdx.qty,
              dataSecondaryIdx.total_price,
              dataSecondaryIdx.short_text,
              dataSecondaryIdx.long_text
            );
          } else {
      ...
      if (
        allMR[i].third_section !== undefined &&
        allMR[i].third_section.service_details !== undefined
      ) {
        for (let j = 0; j < btsSec3; j++) {
          let dataThirdIdx = allMR[i].third_section.service_details[j];
          if (dataThirdIdx !== undefined) {
            rowSec3.push(
              dataThirdIdx.description,
              dataThirdIdx.type_of_cost,
              dataThirdIdx.price
            );
          } else {
      ...
      ws.addRow(
        dsa_mr_row
          .concat(dsa_mr_row_status)
          .concat(dsa_mr_row_das_general)
          .concat(rowSec1)
          .concat(rowSec2)
          .concat(rowSec3)
      );
            ...
            <Card>
              <CardHeader>
                ...
                <Button
                  ...
                  color="success"
                  size="sm"
                  onClick={this.downloadDSAlist}
                >
                  <i
                    className="fa fa-download"
                    style={{ marginRight: "8px" }}
                  ></i>
                  Download DSA ReturnList
                </Button>
                {this.state.userRole.findIndex((e) => e === "Admin") !== -1 && (
                  <Link to={"/dsa-srn-migration"}>
                    <Button
                      color="success"
