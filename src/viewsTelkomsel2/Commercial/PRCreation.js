...
class PRCreation extends Component {
  ...
  getTechBoqList() {
    ...
    this.getDataFromAPINODE(
      '/techBoqList?srt=_id:-1&v={"no_tech_boq" : 1, "_id" : 1}'
    ).then((res) => {
  ...
  saveCommtoAPI = async () => {
    ...
    for (let i = 0; i < dataCommNew.techBoqSite.length; i++) {
      ...
      if (
        dataCommNew.techBoqSite[i].id_service_product_doc !== undefined &&
        dataCommNew.techBoqSite[i].id_service_product_doc !== null
      ) {
        const dataItemNew = {
          pr_item_type: "SVC",
