...
class BulkNotifytoASP extends Component {
  constructor(props) {
    ...
    this.handleChangeFilterUserChecked = this.handleChangeFilterUserChecked.bind(
      this
    );
  }
  ...
  getAssignmentList() {
    ...
    if (this.state.check_userall === false) {
      filter_array.push('"created_by":"' + this.props.dataLogin._id + '"');
    }
  ...
  async requestForNotifyBulk() {
    ...
    const res = await this.patchDatatoAPINODE(
      "/aspAssignment/notifyManyAspAssignment",
      { ids: sucPatch }
    );
    if (res.data !== undefined) {
      //Send Email Vendor
      const dataVendor = await this.getVendorList(
        dataASGChecked.map((e) => e.Vendor_Name)
      );
      for (let i = 0; i < dataVendor.length; i++) {
        let dataAssignmentforVendor = dataASGChecked.filter(
          (e) => e.Vendor_Code_Number === dataVendor[i].Vendor_Code
        );
        let dataTableASGEmail = "";
        dataAssignmentforVendor.map(
          (asg) =>
            (dataTableASGEmail =
              dataTableASGEmail +
              "<tr><td style='padding:5px'><a href='https://dev.bam-id.e-dpm.com/assignment-detail-asp/" +
              asg._id +
              "'>" +
              asg.Assignment_No +
  ...
  handleChangeFilterUserChecked(e) {
    this.setState(
      { check_userall: e.target.checked, assignment_list: [] },
      () => {
        this.getAssignmentList();
      }
    );
          ...
          <Col xs="12" lg="12">
            <Card>
              ...
              <CardBody>
                <div>
                  <Checkbox
                    checked={this.state.check_userall}
                    onChange={this.handleChangeFilterUserChecked}
                  />
                  <span>No Filter Creator</span>
                </div>
                <div>
                  <span>By defualt filter by user creator</span>
                </div>
                ...
                {this.state.data_asg_checked.length !== 0 && (
                  <div>
                    <Button
                      color="success"
                      style={{ float: "right" }}
                      onClick={this.requestForNotifyBulk}
                    >
                      {" "}
                      Send Notify to ASP
                    </Button>
