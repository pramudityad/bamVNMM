...
class TableTechnicalItem extends React.Component {
  ...
  getDataCD(site_id, project_po, project) {
    const dataCDIdx = this.props.dataCD.find(
      (e) =>
        e.CD_Info_Project_Name === project &&
        e.Site_Info_SiteID_NE_Actual === site_id &&
        e.Project_PO === project_po
    );
      ...
      <Table hover bordered striped responsive size="sm">
        ...
        <tbody>
          {this.props.dataTechBoqSites.map((site) => (
            <tr>
              {this.props.boqType === "MW" && <td>{site.link_id}</td>}
              <td>{site.region}</td>
              {this.props.boqType === "MW" && <td>{site.region_fe}</td>}
              <td>{site.program}</td>
              <td>{site.site_id}</td>
              {this.props.boqType === "MW" && <td>{site.site_id_fe}</td>}
              <td>{site.wp_id}</td>
...
class TSSRboq extends Component {
  ...
  getTechBoqData(_id_tech) {
    getDatafromAPINODE(
      "/tssr/getTssrWithDelta/" + _id_tech + "?package=1",
      this.props.dataLogin.token
    ).then((res) => {
      if (res.data !== undefined) {
        const dataTech = res.data;
        this.setState({
          data_tssr_boq: dataTech.data,
          boq_type: dataTech.data.tech_boq_type,
        });
  ...
  viewTechBoqDataISAT(data_sites) {
    if (data_sites !== undefined && data_sites.length !== 0) {
      ...
      this.setState({
        ...
        action_message:
          "There is error, please select other BOQ Implementation",
      });
    }
  }
  ...
  async updateTechBoq(e) {
    ...
    let patchTech = await patchDatatoAPINODE(
  ...
  getVersionTechBoqData(_id_tech, ver) {
    getDatafromAPINODE(
      ...
      if (res.data !== undefined) {
        ...
        if (res.data.data !== undefined) {
          this.setState(
            { data_tssr_boq_sites_version: dataTech.data.techBoqSiteVersion },
            () => {
      ...
      if (res !== undefined) {
        if (res.data !== undefined) {
          this.setState(
            {
              action_message:
                "Your project have been assign to BOQ Implementation ",
  ...
  async checkingFormatTSSR(rowsTech) {
    ...
    // }
    let postCheck = await postDatatoAPINODE(
      urlCheck,
      dataCheck,
      this.props.dataLogin.token
    );
  ...
  saveTechBoq = async () => {
    ...
    let postTech = await postDatatoAPINODE(
  ...
  exportTechnical = async () => {
    ...
    saveAs(
      new Blob([techFormat]),
      "BOQ Implementation " + dataTech.no_tech_boq + " Report.xlsx"
    );
  };
  ...
  exportFormatTechnical = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tech_boq + " Uploader Template.xlsx"
    );
  };
  ...
  getDataCD(site_id, project_po, project) {
    const dataCDIdx = this.state.data_from_cd.find(
      (e) =>
        e.CD_Info_Project_Name === project &&
        e.Site_Info_SiteID_NE_Actual === site_id &&
        e.Project_PO === project_po
  ...
  exportTechnicalHorizontal = async () => {
      ...
      if (this.state.boq_type === "MW") {
        ws.addRow(
          [
            dataSites[i].link_id,
            dataSites[i].region,
            dataSites[i].region_fe,
            dataSites[i].program,
            dataSites[i].batch,
            dataSites[i].site_id,
            dataSites[i].site_name,
            dataSites[i].ne_id,
            dataSites[i].site_id_fe,
            dataSites[i].site_name_fe,
            dataSites[i].fe_id,
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tssr_boq + ".xlsx"
    );
  };
  ...
  exportFormatTechnicalHorizontal = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tssr_boq + " Format.xlsx"
    );
  };
  ...
  async getPPandMaterialTSSR(array_id_pp_doc) {
    ...
    for (let i = 0; i < getNumberPage; i++) {
      ...
      let projection =
        '&v={"pp_id":1,"product_name":1,"product_type":1,"physical_group":1,"materials":1}';
      let where_id_pp_doc = '?q={"_id" : {"$in": ["' + arrayIdppdoc + '"]}}';
      let resPP = await getDatafromAPINODE(
        "/productpackage" + where_id_pp_doc,
  ...
  exportImplementationWithMaterial = async () => {
      ...
      for (let i = 0; i < dataSites.length; i++) {
        ...
        if (
          ...
          for (let j = 0; j < dataSites[i].siteItemVersion.length; j++) {
            ...
            let dataPPDb = dataPPMM.find(
              (dp) => dp._id === dataSPPIdx.id_pp_doc
            );
            if (dataPPDb !== undefined) {
              for (let k = 0; k < dataPPDb.materials.length; k++) {
                let dataMatSIdx = dataPPDb.materials[k];
                const rowData = [
                  dataSPPIdx.project_name,
                  dataSPPIdx.no_tssr_boq,
                  dataSites[i].region,
                  dataSites[i].region_fe,
                  dataSites[i].site_id,
                  dataSites[i].site_name,
                  ...
                  dataSites[i].wp_id,
                  dataSPPIdx.pp_id,
                  dataSPPIdx.product_name,
                  dataSPPIdx.product_type,
                  dataSPPIdx.physical_group,
                  dataSPPIdx.qty,
                  dataSites[i].program,
                  dataMatSIdx.material_id,
                  dataMatSIdx.material_name,
                  dataMatSIdx.uom,
                  dataSPPIdx.qty * dataMatSIdx.qty,
                  dataMatSIdx.material_type,
                ];
                ws.addRow(rowData);
              }
            } else {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tech_boq + " With Material.xlsx"
    );
    this.toggleLoading();
  };
  ...
  exportFormatTechnicalVerticalUploader = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tech_boq + " Vertical Uploader.xlsx"
    );
  };
  ...
  exportFormatTechnicalVertical = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Implementation " + dataTech.no_tech_boq + " Vertical.xlsx"
    );
  };
  ...
  render() {
    if (this.state.redirectSign !== false) {
      return (
        <Redirect
          to={"/detail-BOQ Implementation/" + this.state.redirectSign}
        />
      );
    }
          ...
          <Col xl="12">
            <Card>
                ...
                this.props.match.params.id == undefined ? (
                  ...
                  <React.Fragment>
                    <span style={{ marginTop: "3px", position: "absolute" }}>
                      Detail BOQ Implementation{" "}
                      {this.state.boq_type === "MW" && "Transmission"}
                    </span>
                    <div
                        ...
                        >
                          ...
                          {this.state.data_tssr_boq !== null && (
                            <DropdownMenu>
                              ...
                              <DropdownItem
                                onClick={this.exportTechnicalHorizontal}
                              >
                                {" "}
                                <i
                              ...
                              >
                                ...
                                <i
                                  ...
                                  aria-hidden="true"
                                ></i>{" "}
                                BOQ Implementation Format
                              </DropdownItem>
                              <DropdownItem
                                onClick={this.exportImplementationWithMaterial}
                              >
                                {" "}
                                <i
                                  className="fa fa-file-text-o"
                                  aria-hidden="true"
                                ></i>{" "}
                                BOQ Implementation With Material
                              </DropdownItem>
                            </DropdownMenu>
                          )}
                        </Dropdown>
                      </Col>
                      {(this.state.userRole.includes("BAM-Engineering") ===
                        true ||
              ...
              <CardBody className="card-UploadBoq">
                      ...
                      <React.Fragment>
                        ...
                        <Button
                          ...
                          disabled={
                            ...
                            this.state.result_check_tssr === null
                          }
                        >
                        ...
                        >
                          <tbody>
                            ...
                            >
                              ...
                              >
                                Doc : {this.state.data_tssr_boq.no_tssr_boq}
                              </td>
                            </tr>
                          </tbody>
                        </table>
                          ...
                          >
                            <tbody>
                              ...
                              <Input type="select" value={this.state.version_selected === null? this.state.data_tssr_boq.version : this.state.version_selected} onChange={this.handleChangeVersion} style={{width : "100px", height : "30px"}}>
                                ...
                                <td>{this.state.data_tssr_boq.version}</td>
                              </tr>
                              <tr
                                style={{ fontWeight: "425", fontSize: "15px" }}
                              >
                                ...
                                <td style={{ textAlign: "left" }} colspan={2}>
                                  {convertDateFormatfull(
                                    this.state.data_tssr_boq.created_on
                                  )}
                                </td>
                              </tr>
                              <tr
                              ...
                              >
                                ...
                                <td style={{ textAlign: "left" }} colspan={2}>
                                  {
                                    this.state.data_tssr_boq.creator[
                                      this.state.data_tssr_boq.creator.length -
                                        1
                                    ].email
                                  }
                                </td>
                          ...
                          >
                            <tbody>
                              ...
                              >
                                ...
                                <td style={{ textAlign: "left" }} colspan={2}>
                                  {convertDateFormatfull(
                                    this.state.data_tssr_boq.updated_on
                                  )}
                                </td>
                              </tr>
                              <tr
                              ...
                              >
                                ...
                                <td style={{ textAlign: "left" }} colspan={2}>
                                  {
                                    this.state.data_tssr_boq.updater[
                                      this.state.data_tssr_boq.updater.length -
                                        1
                                    ].email
                                  }
                                </td>
                ...
                {this.state.data_tssr_boq_sites.length === 0 &&
                this.state.data_tssr_boq === null &&
                this.props.match.params.id === undefined ? (
                  <Table hover bordered striped responsive size="sm">
                    <tbody>
                  ...
                  <React.Fragment>
                    ...
                    this.state.data_tssr_boq.version !==
                      ...
                      <TableTechnicalItem
                        dataTechBoqSites={
                          this.state.data_tssr_boq_sites_version
                        }
                        TechHeader={
                    ...
                    <nav>
                      <div>
                        <Pagination
                          ...
                          totalItemsCount={
                            this.state.data_tssr_boq_sites.length
                          }
          ...
          <ModalBody>
            <div style={{ textAlign: "center", margin: "15px 0 20px 0" }}>
              <span style={{ fontWeight: "500", fontSize: "15px" }}>
                Update this BOQ Implementation can delete Commercial BOQ related
                with this BOQ Implementation{" "}
              </span>
            </div>
            <div style={{ textAlign: "center", margin: "15px 0 20px 0" }}>
