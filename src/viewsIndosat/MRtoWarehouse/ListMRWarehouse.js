...
class ListMRWarehouse extends Component {
  constructor(props) {
    super(props);
  ...
  getPSDisList() {
    ...
    this.getDataFromAPINODE('/matreq?srt=_id:-1&q=' + whereAnd + '&lmt=' + maxPage + '&pg=' + page).then(res => {
      if (res.data !== undefined) {
        ...
        this.setState({ mr_wh_list: items, totalData: totalData });
      }
    })
  }
            ...
            <Card>
              <CardHeader>
                <span style={{ lineHeight: '2' }}>
                  <i className="fa fa-align-justify" style={{ marginRight: "8px" }}></i> MR Warehouse List
                </span>
                {((this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-Mover") === -1)) && (
                  ...
                  <Dropdown size="sm" isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{ float: 'right', marginRight : '10px' }}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownItem onClick={this.downloadPSDisList}> <i className="fa fa-file-text-o" aria-hidden="true"></i>Download MR WH List</DropdownItem>
                    </DropdownMenu>
                  </Dropdown>
                </React.Fragment>
                )}
                <Link to={'/mr-creation'}><Button color="success" style={{ float: 'right', marginRight: "8px" }} size="sm"><i className="fa fa-plus-square" style={{ marginRight: "8px" }}></i>Create MR Warehouse </Button></Link>
              </CardHeader>
              <CardBody>
                <Table responsive striped bordered size="sm">
                  ...
                  <tbody>
                    {this.state.mr_wh_list.length === 0 && (
                      <tr>
                        <td colSpan="15">No Data Available</td>
                      </tr>
                    )}
                    {this.state.mr_wh_list.map((list, i) =>
                      <tr key={list._id}>
...
export default connect(mapStateToProps, mapDispatchToProps)(ListMRWarehouse);
