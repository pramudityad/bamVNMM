...
class MRWHNAList extends Component {
  ...
  async downloadPSDisList() {
    ...
    let headerRow = ["SHF No.", "Related To", "Related No.", "Tower ID", "Tower Name", "Latitude", "Longitude", "Distance", "Amount Paid", "Receiver Name", "Receiver Email", "Note", "Receiver Phone", "Created By", "Created On"];
    ws.addRow(headerRow);
            ...
            <Card>
              <CardHeader>
                <span style={{ lineHeight: '2' }}>
                  <i className="fa fa-align-justify" style={{ marginRight: "8px" }}></i> MR Warehouse Need Assign List
                </span>
                {((this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-Mover") === -1)) && (
                  ...
                  <Dropdown size="sm" isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{ float: 'right', marginRight : '10px' }}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownItem onClick={this.downloadPSDisList}> <i className="fa fa-file-text-o" aria-hidden="true"></i>Download MR Warehouse List</DropdownItem>
                    </DropdownMenu>
                  </Dropdown>
                </React.Fragment>
                )}
                ...
                <Table responsive striped bordered size="sm">
                  ...
                  <tbody>
                    ...
                    {this.state.mr_dis_list.map((list, i) =>
                      <tr key={list._id}>
                        <td><Link to={'/toWH/mr-wh-na-detail/'+list._id}><Button size="sm" outline color="info">Assign</Button></Link></td>
