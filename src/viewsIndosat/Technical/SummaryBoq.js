...
class TableTechnicalItem extends React.Component {
  ...
  getSummaryRow(row_data_item, region, program) {
    let qtyIdx;
    const dataItemIdx = row_data_item.qty_side.filter(
      (e) => e.region === region
    );
    if (dataItemIdx !== undefined) {
      if (program !== undefined) {
        ...
        const dataItemIdxReg = dataItemIdx.find(
          (e) => e.region === region && e.program === undefined
        );
          ...
          {this.props.productType.map((pt) =>
            this.filterPerProductType(pt).map((item) => (
              <tr>
                ...
                {this.props.TechHeader.region.map((region) => (
                  <Fragment>{this.getSummaryRow(item, region)}</Fragment>
                ))}
                {this.props.TechHeader.program.map((program) => (
                  <Fragment>
                    {this.props.TechHeader.region.map((region) => (
...
class SummaryBoq extends Component {
  constructor(props) {
    ...
    this.exportFormatTechnicalCommercial = this.exportFormatTechnicalCommercial.bind(
      this
    );
    ...
    this.exportFormatTechnicalVertical = this.exportFormatTechnicalVertical.bind(
      this
    );
    this.approvalTechnical = this.approvalTechnical.bind(this);
    this.handleChangeOptionView = this.handleChangeOptionView.bind(this);
    this.handleChangeFormatUploader = this.handleChangeFormatUploader.bind(
      this
    );
  ...
  getTechBoqData(_id_tech) {
    this.getDataFromAPINODE("/techBoq/" + _id_tech + "?package=1").then(
      (res) => {
  ...
  viewTechBoqDataISAT(data_sites) {
    if (data_sites !== undefined && data_sites.length !== 0) {
      const dataTechItem = data_sites
        .map((e) => e.siteItem)
        .reduce((l, n) => l.concat(n), []);
      const dataTechItemNSVC = dataTechItem.filter(
        (e) => e.product_type !== "SVC"
      );
      let techItemUniqID = [
        ...new Set(dataTechItemNSVC.map(({ pp_id }) => pp_id)),
      ];
      let techItemUniqType = [];
      let techItemUniqName = [];
      for (let i = 0; i < techItemUniqID.length; i++) {
        let itemTechFind = dataTechItemNSVC.find(
          (e) => e.pp_id === techItemUniqID[i]
        );
        techItemUniqType.push(itemTechFind.product_type);
        techItemUniqName.push(itemTechFind.product_name);
      }
      const Header = {
        type: techItemUniqType,
        name: techItemUniqName,
        pp_id: techItemUniqID,
      };
      ...
      this.setState({
        action_status: "failed",
        action_message: "There is error, please select other TSSR BOQ",
      });
    }
  }
  ...
  viewTechBoqDataSummary(data_sites) {
    if (data_sites !== undefined && data_sites.length !== 0) {
      ...
      let product_type_uniq = [
        ...new Set(dataTechItem.map(({ product_type }) => product_type)),
      ];
      product_type_uniq.push("Service");
      let techRegionUniq = [...new Set(data_sites.map(({ region }) => region))];
      let techProgramUniq = [
        ...new Set(data_sites.map(({ program }) => program)),
      ];
      let techServiceUniqID = [
        ...new Set(
          data_sites.map(({ service_product_id }) => service_product_id)
        ),
      ];
      let techItemUniqID = [
        ...new Set(dataTechItemNSVC.map(({ pp_id }) => pp_id)),
      ...
      for (let i = 0; i < techRegionUniq.length; i++) {
        let dataFilterIdx = {};
        let regionProgram = [];
        let sitesTechFilterRegion = data_sites.filter(
          (e) => e.region === techRegionUniq[i]
        );
        const itemTechFilterRegion = sitesTechFilterRegion
          .map((e) => e.siteItem)
          .reduce((l, n) => l.concat(n), []);
        ...
        for (let j = 0; j < techProgramUniq.length; j++) {
          ...
          let sitesTechFilterRegionProgram = sitesTechFilterRegion.filter(
            (e) => e.program === techProgramUniq[j]
          );
          const itemTechFilterRegionProgram = sitesTechFilterRegionProgram
            .map((e) => e.siteItem)
            .reduce((l, n) => l.concat(n), []);
      ...
      for (let i = 0; i < techItemUniqID.length; i++) {
        ...
        let itemTechFilter = dataTechItemNSVC.filter(
          (e) => e.pp_id === techItemUniqID[i]
        );
        ...
        dataSummaryIdx["total_qty"] = itemTechFilter.reduce(
          (a, b) => a + b.qty,
          0
        );
        for (let j = 0; j < dataTechSitePerRegional.length; j++) {
          let regionItemIdx = dataTechSitePerRegional[j];
          let qtyItmReg = regionItemIdx.item_list
            .filter((e) => e.pp_id === techItemUniqID[i])
            .reduce((a, b) => a + b.qty, 0);
          dataQtyRegion.push({ region: techRegionUniq[j], qty: qtyItmReg });
          for (let k = 0; k < regionItemIdx.region_program.length; k++) {
            const qtyItmRegProgram = regionItemIdx.region_program[k].item_list
              .filter((e) => e.pp_id === techItemUniqID[i])
              .reduce((a, b) => a + b.qty, 0);
            dataQtyRegion.push({
              region: techRegionUniq[j],
              qty: qtyItmReg,
              program: regionItemIdx.region_program[k].program,
            });
          }
        }
      ...
      for (let i = 0; i < techServiceUniqID.length; i++) {
        let dataSummaryIdx = {};
        let dataQtyRegion = [];
        let itemTechFilterSVC = data_sites.filter(
          (e) => e.service_product_id === techServiceUniqID[i]
        );
        dataSummaryIdx["pp_id"] = techServiceUniqID[i];
        dataSummaryIdx["product_type"] = "Service";
        dataSummaryIdx["product_name"] =
          itemTechFilterSVC[0].service_product_name;
        dataSummaryIdx["source_material"] = null;
        dataSummaryIdx["total_qty"] = itemTechFilterSVC.length;
        for (let j = 0; j < dataTechSitePerRegional.length; j++) {
          let regionItemIdx = dataTechSitePerRegional[j];
          let qtyItmReg = regionItemIdx.site_list.filter(
            (e) => e.service_product_id === techServiceUniqID[i]
          ).length;
          dataQtyRegion.push({ region: techRegionUniq[j], qty: qtyItmReg });
          for (let k = 0; k < regionItemIdx.region_program.length; k++) {
            const qtyItmRegProgram = regionItemIdx.region_program[
              k
            ].site_list.filter(
              (e) => e.service_product_id === techServiceUniqID[i]
            ).length;
      ...
      this.setState(
        {
          view_tech_header_table: Header,
          data_tech_boq_summary: dataSummary,
          product_type_uniq: product_type_uniq,
        },
        () => {
  ...
  getVersionTechBoqData(_id_tech, ver) {
    this.getDataFromAPINODE(
      "/techBoq/" + _id_tech + "/ver/" + ver + "?package=1"
    ).then((res) => {
      if (res.data !== undefined) {
        const dataTech = res.data;
        if (res.data.data !== undefined) {
          this.setState(
            { data_tech_boq_summary_version: dataTech.data.techBoqSiteVersion },
            () => {
  ...
  exportTechnical = async () => {
    ...
    const rowHeader2 = ["Tower ID", "Tower Name"].concat(
    ...
    for (let i = 0; i < dataSites.length; i++) {
      ws.addRow(
        [dataSites[i].site_id, dataSites[i].site_name].concat(
          dataSites[i].siteItemConfig.map((e) => e.qty)
        )
      );
    }
  ...
  exportTechnicalSummaryBOQ = async () => {
    ...
    for (let i = 0; i < dataProductTypeUniq.length; i++) {
      const dataSummaryType = dataTechSummary.filter(
        (ts) => ts.product_type === dataProductTypeUniq[i]
      );
      for (let j = 0; j < dataSummaryType.length; j++) {
        let listRow = [
          dataSummaryType[j].product_type,
          dataSummaryType[j].product_name,
          dataSummaryType[j].ordering,
          dataSummaryType[j].inf_code,
          dataSummaryType[j].total_qty,
        ];
        for (let k = 0; k < dataSummaryHeader.region.length; k++) {
          let qtyIdx = 0;
          const qtySiteIdx = dataSummaryType[j].qty_side.find(
            (qs) =>
              qs.region === dataSummaryHeader.region[k] &&
              qs.program === undefined
          );
    ...
    saveAs(
      new Blob([MRFormat]),
      "Technical Summary BOQ " + dataTech.no_tech_boq + ".xlsx"
    );
  };
  ...
  exportFormatTechnicalHorizontal = async () => {
    ...
    header_config.pp_id.map((e, i) =>
      HeaderRow2.push(e + " /// " + header_config.name[i])
    );
    ...
    for (let i = 0; i < dataSites.length; i++) {
      ...
      if (
        ...
        for (let j = 0; j < header_config.pp_id.length; j++) {
          let dataItemIdx = dataSites[i].siteItemVersion.find(
            (e) => e.pp_id === header_config.pp_id[j]
          );
          if (dataItemIdx !== undefined) {
        ...
        for (let j = 0; j < header_config.pp_id.length; j++) {
          let dataItemIdx = dataSites[i].siteItem.find(
            (e) => e.pp_id === header_config.pp_id[j]
          );
          if (dataItemIdx !== undefined) {
            qtyItem.push(dataItemIdx.qty);
      ...
      ws.addRow(
        [
          ...
          dataSites[i].ne_id,
          dataSites[i].new_config,
          dataSites[i].service_product_id,
          dataSites[i].service_product_name,
        ].concat(qtyItem)
      );
    }
    ...
    saveAs(
      new Blob([MRFormat]),
      "Technical BOQ " + dataTech.no_tech_boq + " Format.xlsx"
    );
  };
  ...
  getTaxCode(product_type) {
    if (
      product_type.toLowerCase() === "lcm" ||
      product_type.toLowerCase() === "svc"
    ) {
      return "V1";
    } else {
      return "V0";
  ...
  saveCommtoAPI = async () => {
    ...
    let dataTechperRegion = dataCommNew.techBoqSite.filter(
      (ts) => ts.region === regionSelected
    );
    let dataItemsPR = [];
    for (let i = 0; i < dataTechperRegion.length; i++) {
      ...
      if (
        dataTechperRegion[i].id_service_product_doc !== undefined &&
        dataTechperRegion[i].id_service_product_doc !== null
      ) {
        const dataItemNew = {
          pr_item_type: "SVC",
    ...
    let postComm = await this.postDatatoAPINODE(
      "/commBoqIsat/createCommercial",
      { data: dataInputan }
    );
          ...
          <Col xl="12">
            <Card>
                      ...
                      <Col>
                        {this.state.data_tech_boq !== null && (
                          <Button
                            ...
                            color="success"
                            value={"1"}
                            disabled={this.state.region_selected === null}
                            onClick={this.saveCommtoAPI}
                          >
                            Create PR
                          </Button>
                        )}
                      ...
                      <Col>
                        ...
                        >
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownItem
                              onClick={this.exportTechnicalSummaryBOQ}
                            >
                              {" "}
                              <i
              ...
              <CardBody className="card-UploadBoq">
                ...
                {this.state.data_tech_boq_summary.length === 0 &&
                this.state.data_tech_boq === null &&
                this.props.match.params.id === undefined ? (
                  <Table hover bordered striped responsive size="sm">
                    <tbody>
                  ...
                  <React.Fragment>
                    <TableTechnicalItem
                      dataTechBoqItem={
                        this.state.data_tech_boq_summary_pagination
                      }
                    ...
                    <nav>
                      <div>
                        <Pagination
                          ...
                          totalItemsCount={
                            this.state.data_tech_boq_summary.length
                          }
