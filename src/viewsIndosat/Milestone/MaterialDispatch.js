...
class MaterialDispatch extends Component {
  ...
  getMRList() {
    ...
    filter_array.push(
      '"$or" : [{"current_milestones": "MS_DISPATCH"}, {"current_milestones": "MS_MATERIAL_ON_SITE"}]'
    );
  ...
  async downloadMRlist() {
    ...
    saveAs(new Blob([allocexport]), "MR List Dispatch.xlsx");
  }

  async getDataWH(wh_id) {
    let resWH = await this.getDataFromAPINODE(
      '/whManagement/warehouse?q={"wh_id" : "' +
        wh_id +
        '"}&v={"wh_id":1,"factory_code":1}'
    );
                ...
                <Table responsive striped bordered size="sm">
                  ...
                  <tbody>
                    ...
                    {this.state.mr_list.map((list, i) => (
                      <tr key={list._id}>
                        <td>
                          {list.current_mr_status ===
                          "LOADING PROCESS FINISH" ? (
                            "Waiting Dispatch"
                          ) : (
                            <React.Fragment>
                              Finish{" "}
                              {this.state.userRole.indexOf(
                                "BAM-ASP Management"
                              ) === -1 &&
                                this.state.userRole.indexOf("BAM-ASP") === -1 &&
                                this.state.userRole.indexOf("BAM-Mover") ===
                                  -1 && (
                                  <Dropdown
                                    size="sm"
