...
class TSSRboq extends Component {
  ...
  viewTechBoqDataISAT(data_sites) {
    if (data_sites !== undefined && data_sites.length !== 0) {
      ...
      this.setState({
        ...
        action_message: "There is error, please select other BOQ Reservation",
      });
    }
  }

  async getDataSystemKey(array_dataSite) {
    let arraySite = array_dataSite.filter(
      (ads) =>
        ads.project_po !== undefined &&
        ads.project_po !== null &&
        ads.project_po.length !== 0
    );
    let array_or = [];
    let dataSite = [];
    ...
    for (let i = 0; i < getNumberPage; i++) {
      ...
      DataPaginationSite.map((ais) =>
        array_or.push(
          '{"Project_PO" : "' +
            ais.project_po +
            '", "CD_Info_Project_Name" : "' +
            ais.project_name +
            '","Site_Info_SiteID_NE_Actual" : "' +
            ais.site_id +
            '"}'
        )
      );
      ...
      let where_id_Site =
        '?where={"$or": [' +
        arrayIdSite +
        ']}&projection={"Project_PO":1,"CD_Info_Project_Name":1,"Site_Info_SiteID_NE_Actual":1,"CD_Info_System_Key":1}';
      ...
      if (res !== undefined) {
        if (res.data !== undefined) {
          this.setState(
            {
              action_message:
                "Your project have been assign to BOQ Reservation ",
  ...
  exportTechnical = async () => {
    ...
    ws.mergeCells("A9:I9");
    ws.getCell("A9").value = "BOQ Reservation";
    ...
    saveAs(
      new Blob([techFormat]),
      "BOQ Reservation " + dataTech.no_tech_boq + " Report.xlsx"
    );
  };
  ...
  exportFormatTechnical = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Reservation " + dataTech.no_tech_boq + " Uploader Template.xlsx"
    );
  };
  ...
  exportTechnicalHorizontal = async () => {
    ...
    if (this.state.boq_type === "MW") {
      ...
      let HeaderRow2 = [
        ...
        "service_product_id_fe",
        "service_product_name_fe",
      ];
      ...
      if (this.state.boq_type === "MW") {
        ws.addRow(
          [
            ...
            dataSites[i].project_po,
            this.getDataCD(
              dataSites[i].site_id,
              dataSites[i].project_po,
              dataSites[i].project_name
            ),
            dataSites[i].ne_id,
            dataSites[i].fe_id,
            ...
            dataSites[i].service_product_id_fe,
            dataSites[i].service_product_name_fe,
          ].concat(qtyItem)
        );
      } else {
        ws.addRow(
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Reservation " + dataTech.no_tssr_boq + ".xlsx"
    );
  };
  ...
  exportFormatTechnicalHorizontal = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Reservation " + dataTech.no_tssr_boq + " Format.xlsx"
    );
  };
  ...
  exportFormatTechnicalVerticalUploader = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Reservation " + dataTech.no_tech_boq + " Vertical Uploader.xlsx"
    );
  };
  ...
  exportFormatTechnicalVertical = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "BOQ Reservation " + dataTech.no_tech_boq + " Vertical.xlsx"
    );
  };
  ...
  render() {
    if (this.state.redirectSign !== false) {
      return (
        <Redirect to={"/detail-BOQ Reservation/" + this.state.redirectSign} />
      );
    }
            ...
            <Card>
              <CardHeader>
                ...
                this.props.match.params.id == undefined ? (
                  <React.Fragment>
                    <span>Create BOQ Reservation</span>
                  </React.Fragment>
                ) : (
                  <React.Fragment>
                    <span style={{ marginTop: "3px", position: "absolute" }}>
                      Detail BOQ Reservation{" "}
          ...
          <ModalBody>
            <div style={{ textAlign: "center", margin: "15px 0 20px 0" }}>
              <span style={{ fontWeight: "500", fontSize: "15px" }}>
                Update this BOQ Reservation can delete Commercial BOQ related
                with this BOQ Reservation{" "}
              </span>
            </div>
            <div style={{ textAlign: "center", margin: "15px 0 20px 0" }}>
