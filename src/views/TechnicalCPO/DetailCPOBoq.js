...
class DetailCPOBoq extends Component {
    ...
    this.getDataFromAPINODE("/techBoq/" + _id_tech).then((res) => {
      if (res.data !== undefined) {
        ...
        if (res.data.data !== undefined) {
          this.setState(
            {
              data_cpo_boq_sites: dataTech.data.techBoqSite,
  ...
  async updateTechBoq(e) {
    ...
    let patchTech = await this.patchDatatoAPINODE(
      "/techBoq/updateTechBoqData/" + this.state.data_cpo_boq._id,
      dataPatch
    );
    ...
    this.getDataFromAPINODE("/techBoq/" + _id_tech + "/ver/" + ver).then(
      (res) => {
        if (res.data !== undefined) {
          ...
          if (res.data.data !== undefined) {
            this.setState(
              { data_cpo_boq_sites_version: dataTech.data.techBoqSiteVersion },
              () => {
  ...
  async saveCPOBOQ() {
    ...
    for (let j = 1; j < dataXLS.length; j++) {
      ...
      let cpoIdx = {
        ...
        coupa_catalogue_item:
          xlsRow[this.getIndex(dataXLS[0], "coupa_catalogue_item")],
        coupa_unit_price: xlsRow[this.getIndex(dataXLS[0], "coupa_unit_price")],
        coupa_total_price:
          xlsRow[this.getIndex(dataXLS[0], "coupa_total_price")],
        currency: xlsRow[this.getIndex(dataXLS[0], "currency")],
        coupa_requisition_title:
          xlsRow[this.getIndex(dataXLS[0], "coupa_requisition_title")],
        ...
        pcode_6967_pndt_design_tuning:
          xlsRow[this.getIndex(dataXLS[0], "Pcode 6967-PNDT Design-Tuning")],
        pcode_6967_cos: xlsRow[this.getIndex(dataXLS[0], "Pcode 6967-CoS %")],
        pcode_2961_nro_total_value:
          xlsRow[this.getIndex(dataXLS[0], "Pcode1 Total Value (2961-NRO)")],
        pcode_6967_pndt_design_tuning_total_value:
          xlsRow[
            this.getIndex(
              dataXLS[0],
              "Pcode2 Total Value (6967-PNDT) Design-Tuning"
            )
          ],
        pcode_6967_cos_total_value:
          xlsRow[this.getIndex(dataXLS[0], "Pcode4 Total Value (6967-CoS)")],
        ...
        pcode_hw_2460_total_value:
          xlsRow[this.getIndex(dataXLS[0], "Pcode HW- 2460 Total Value")],
        pcode_lcm_2110_total_value:
          xlsRow[this.getIndex(dataXLS[0], "Pcode LCM - 2110 Total Value")],
        pcode_eab_hwac_2491_total_value:
          xlsRow[
            this.getIndex(dataXLS[0], "Pcode EAB HWAC - 2491 Total Value")
          ],
      };
      if (
        cpoIdx.coupa_unit_price !== undefined &&
        cpoIdx.coupa_unit_price !== null
      ) {
        cpoIdx["coupa_unit_price"] = parseFloat(cpoIdx.coupa_unit_price);
      }
      if (
        cpoIdx.coupa_total_price !== undefined &&
        cpoIdx.coupa_total_price !== null
      ) {
    ...
    let postData = await this.postDatatoAPINODE("/cpoBoq/createCpo", {
      cpoBoqData: cpoData,
    });
          ...
          if (postData.response.data.error !== undefined) {
            if (postData.response.data.error.message !== undefined) {
              this.setState(
                {
                  action_status: "failed",
                  action_message: postData.response.data.error.message,
                },
                () => {
                  this.toggleLoading();
                }
              ...
              this.setState(
                {
                  action_status: "failed",
                  action_message: postData.response.data.error,
                },
                () => {
                  this.toggleLoading();
                }
  ...
  async updateCPOBOQ() {
    ...
    let postData = await this.patchDatatoAPINODE(
      "/cpoBoq/updateCpo/" + this.state.data_cpo_boq[0].cpo_number,
      { cpoBoqData: cpoData }
    );
    if (postData.data !== undefined) {
      this.setState({ action_status: "success" });
  ...
  async approvalTechnical(e) {
    ...
    let patchData = await this.patchDatatoAPINODE(
      "/techBoq/approval/" + this.state.data_cpo_boq._id,
      { operation: currValue }
    );
    ...
    this.getDataFromAPINODE("/commBoq?srt=_id:-1").then((res) => {
      if (res.data !== undefined) {
        ...
        if (dataCPO.data !== undefined) {
          const dataListSubmission = dataCPO.data
            .map((value) =>
              value.comm_boq_submission_doc
                .filter((e) => e.submission_status !== "CANCELED")
                .map((child) => child)
            )
            .reduce((l, n) => l.concat(n), []);
  ...
  async checkingFormatTech(rowsCPO) {
    let postCheck = await this.postDatatoAPINODE("/techBoq/checkTechBoqData", {
      techBoqData: rowsCPO,
    });
    if (postCheck.data !== undefined) {
      ...
      if (postCheck.response !== undefined) {
        if (postCheck.response.error !== undefined) {
          this.setState({
            action_status: "failed",
            action_message: postCheck.response.error,
          });
        } else {
          this.setState({ action_status: "failed" });
        }
  ...
  async compareWithSubmission() {
    ...
    let postData = await this.getDataFromAPINODE(
      "/cpoBoq/compareWithCommBoqSubmission/" +
        cpoNumber +
        "/commBoq/" +
        submission_id
    );
    if (postData.data !== undefined) {
      this.setState({
        data_cpo_boq: postData.data.data,
        data_updateSap: postData.data.updateSap,
      });
    }
    this.toggleLoading();
  }
  ...
  async assignSubmissionToCPO() {
    ...
    let submission_id = this.state.submission_number_selected;
    let postData = await this.patchDatatoAPINODE(
      "/cpoBoq/assignCpoToCommBoqSubmission/" +
        cpoNumber +
        "/commBoq/" +
        submission_id,
      { updateSap: this.state.data_updateSap }
    );
    if (postData.data !== undefined) {
      alert("Success");
  ...
  exportFormatCPO = async () => {
    ...
    const ppIdRow = [
      ...
      "as_number",
      "Pcode 2961-NRO %",
      "Pcode 6967-PNDT Design-Tuning",
      "Pcode 6967-CoS %",
      "Pcode1 Total Value (2961-NRO)",
      "Pcode2 Total Value (6967-PNDT) Design-Tuning",
      "Pcode4 Total Value (6967-CoS)",
      "HW- 2460",
      "LCM - 2110",
      "EAB HWAC - 2491",
      "Pcode HW- 2460 Total Value",
      "Pcode LCM - 2110 Total Value",
      "Pcode EAB HWAC - 2491 Total Value",
    ];
    ws.addRow(ppIdRow);
    ws.addRow([
      "EXAMPLE PO0001",
      ...
      280000,
      "IDR",
      "EID-2020CUP_RAN_Service_ERICSSON_1_Central",
      "2020CUP_RAN_3416315_ERICSSON_1_Central",
      1490,
      "AS#0223",
      "100%",
      "0%",
  ...
  exportFileCPOBOQ = async () => {
    ...
    let ppIdRow = [
      ...
      "Category",
      "Source",
      "COUPA Catalogue Item",
      "Coupa Unit Price",
      "Coupa Total Price",
      "Curr",
      "COUPA Requisition Title",
      "COUPA Network ID",
      "PR#",
      "AS Number",
      "Pcode 2961-NRO %",
    ...
    for (let i = 0; i < dataCPO.length; i++) {
      ...
      let rowBody = [
        ...
        row.category,
        row.source,
        row.coupa_catalogue_item,
        row.coupa_unit_price,
        row.coupa_total_price,
        row.currency,
        row.coupa_requisition_title,
        row.coupa_network_id,
        row.pr_number,
        row.as_number,
        row.pcode_2961_nro,
        row.pcode_6967_pndt_design_tuning,
        row.pcode_6967_cos,
        row.pcode_2961_nro_total_value,
        row.pcode_6967_pndt_design_tuning_total_value,
        row.pcode_6967_cos_total_value,
        row.hw_2460,
        row.lcm_2110,
        row.eab_hwac_2491,
        row.pcode_hw_2460_total_value,
        row.pcode_lcm_2110_total_value,
        row.pcode_eab_hwac_2491_total_value,
      ];
      if (this.state.submission_number_selected !== null) {
  ...
  exportFileCPOBOQUpdate = async () => {
    ...
    saveAs(
      new Blob([MRFormat]),
      "CPO BOQ " + dataCPO[0].cpo_number + " Format.xlsx"
    );
  };
            ...
            <Card>
                      ...
                      <Col>
                        ...
                        >
                          <DropdownToggle caret color="secondary">
                            ...
                            Download CPO BQO File
                          </DropdownToggle>
                          <DropdownMenu>
                            <DropdownItem header> CPO File</DropdownItem>
                            <DropdownItem onClick={this.exportFormatCPO}>
                              {" "}
                              <i
                        ...
                        >
                          ...
                          >
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownItem onClick={this.exportFileCPOBOQ}>
                                {" "}
                                <i
                                  className="fa fa-file-text-o"
                                  aria-hidden="true"
                              ...
                              <DropdownItem
                                onClick={this.exportFileCPOBOQUpdate}
                              >
                                {" "}
                                <i
                                  className="fa fa-file-text-o"
                          ...
                          <div>
                            <Button
                              ...
                              color="warning"
                              size="sm"
                              onClick={this.toggleCollapseCompare}
                              id="toggleCollapse1"
                            >
                              Compare
                            </Button>
                ...
                <Card style={{ margin: "10px 10px 5px 10px" }}>
                  <CardBody>
                    <div>
                      <input
                        type="file"
                        onChange={this.fileHandlerCPO.bind(this)}
                        style={{
                      ...
                      <Button
                        ...
                        color="success"
                        onClick={this.updateCPOBOQ}
                        disabled={
                          this.state.action_status === "failed" ||
                          this.state.result_check_tech === 0
                        }
                      >
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td style={{ marginLeft: "20px" }}>
                              <Button
                                size="sm"
                                onClick={this.compareWithSubmission}
                            ...
                            {this.state.data_updateSap !== null && (
                              <td style={{ marginLeft: "20px" }}>
                                <Button
                                  size="sm"
                                  color="success"
                                  onClick={this.assignSubmissionToCPO}
                                  style={{ marginLeft: "15px" }}
                                >
                                  Assign
                                </Button>
                        ...
                        >
                          <tbody>
                            ...
                            >
                              ...
                              >
                                PO Number : {this.props.match.params.id}
                              </td>
                            </tr>
                          </tbody>
                        </table>
                          ...
                          <tr>
                            {this.state.submission_number_selected !== null && (
                              <td
                                className={
                                  "status-cpo-boq-row--" +
                                  this.changeToClassname(row.match_status)
                                }
                              >
                                {row.match_status}
                              </td>
                            ...
                            {this.state.userRole.length !== 0 &&
                              this.state.userRole.includes(
                                ...
                                <React.Fragment>
                                  <td>
                                    {numberWithCommas(row.coupa_unit_price)}
                                  </td>
                                  <td>
                                    {numberWithCommas(row.coupa_total_price)}
                                  </td>
                                </React.Fragment>
                              )}
                            <td>{row.currency}</td>
              ...
              <CardFooter>
                {this.state.data_cpo_boq !== null && (
                  <Row>
                    <Col>
                      {this.state.data_cpo_boq.approval_status ===
                        "PRE APPROVAL" && (
                        <Button
                          size="sm"
                          className="btn-success"
                          ...
                          disabled={
                            !this.state.API_Tech.approval_status ===
                            "PRE APPROVAL"
                          }
                        >
                          {this.state.data_cpo_boq.approval_status ===
