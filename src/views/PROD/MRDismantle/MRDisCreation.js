...
class MRDisCreation extends Component {
  ...
  async loadOptionsWPID(inputValue) {
    if (!inputValue || inputValue.length < 3) {
      ...
      const getTowerID = await getDatafromAPIEXEL(
        '/custdel_op?where={"WP_ID":{"$regex":"' +
          inputValue +
          '", "$options":"i"}}&projection={"WP_ID":1,"WP_Name":1,"CD_Info_Project_Name":1,"CD_Info_Project":1,"Tower_Info_TowerID_NE":1,"Tower_Info_TowerName_NE":1}'
      );
      if (getTowerID !== undefined && getTowerID.data !== undefined) {
        ...
        getTowerID.data._items.map((wp) =>
          tower_id_list.push({
            ...
            site_id: wp.Tower_Info_TowerID_NE,
            site_name: wp.Tower_Info_TowerName_NE,
          })
        );
      }
