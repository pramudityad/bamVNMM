...
class DSADDetail extends Component {
  ...
  async patchDataFiletoAPINode(url, file, _etag){
    try {
      let respond = await axios.patch(API_URL_NODE +url, file, {
        headers: {
          'Authorization': 'Bearer ' + this.state.tokenUser
        },
      })
  ...
  getDSAFile = async (e) => {
    ...
    if(data_dsa.dsa_documents !== undefined && data_dsa.dsa_documents.length !== 0)  {
      const resFile = await getDatafromAPINODEFile('/matreq/getDsaDocument/' + data_dsa._id+'?mrCategory=Return', this.props.dataLogin.token, data_dsa.dsa_documents[0].mime_type);
  ...
  submitGRNumber= async (e) =>{
    let res = await this.patchDatatoAPINODE('/matreq/updateGRDsa/' + this.props.match.params.id, {"mrCategory" : "Return", data : this.state.gr_data});
    if (res.data !== undefined) {
      this.setState({ action_status: "success" });
    } else {
      if (res.response !== undefined && res.response.data !== undefined && res.response.data.error !== undefined) {
  ...
  render() {
    return (
      <div className="animated fadeIn" style={{ overflow: "scroll" }}>
                          ...
                          <Row>
                            ...
                            <Col md="3">
                              <FormGroup style={{ paddingLeft: "16px" }}>
                                ...
                                <Input type="text" readOnly value={this.state.data_dsa.current_mra_status}></Input>
                              </FormGroup>
                            </Col>
                          </Row>
        ...
        {/* Modal Reject DSA */}
        <Modal isOpen={this.state.rejectModal} toggle={this.toggleRejectModal} className={"modal-sm"} >
          <ModalHeader>Reject DSA</ModalHeader>
          <ModalBody>
            <div>
              Are You Sure Reject this DSA?
            </div>
            <Input
              type="textarea"
              rows="9"
        ...
        {/* Modal Approve DSA */}
        <Modal isOpen={this.state.approveModal} toggle={this.toggleApproveModal} className={"modal-sm"} >
          <ModalHeader>Approve DSA</ModalHeader>
          <ModalBody>
            <div>
              Are You Sure Approve this DSA?
            </div>
          </ModalBody>
          <ModalFooter>
            <Button
