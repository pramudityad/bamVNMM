    ...
    this.patchDatatoAPI('/boq_tech_op/'+dataTech._id, data_Project, dataTech._etag).then( res => {
      if(res !== undefined){
        if(res.data !== undefined){
          this.setState({action_message : "Your Project have been assign to Technical", action_status : 'success'});
        }
      }
    })
  }
                          ...
                          <tr style={{fontWeight : '425', fontSize : '15px'}}>
                            ...
                            <td style={{textAlign : 'left'}} colspan={2}>
                              ...
                              {this.state.list_version.map(ver =>
                                <option value={ver.version}>{(ver.rev !== "A" ? "PA" : "A")+ver.version}</option>
                                )}
