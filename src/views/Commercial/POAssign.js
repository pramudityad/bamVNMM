...
class POAssign extends Component {
  ...
  exportCommercial = async () => {
    ...
    const dataGroup = this.state.groupingView;
    const prod_type = [
      ...new Set(dataPrint.map(({ product_type }) => product_type)),
    ];
    ...
    if (
      (this.checkValuetoString(dataComm.note_1).length !== 0 &&
        this.checkValuetoString(dataComm.note_name_1).length !== 0) ||
      (this.checkValuetoString(dataComm.note_2).length !== 0 &&
        this.checkValuetoString(dataComm.note_name_2).length !== 0)
    ) {
    ...
    if (
      (this.checkValuetoString(dataComm.note_3).length !== 0 &&
        this.checkValuetoString(dataComm.note_name_3).length !== 0) ||
      (this.checkValuetoString(dataComm.note_4).length !== 0 &&
        this.checkValuetoString(dataComm.note_name_4).length !== 0)
    ) {
      let getlastnullrow = ws.lastRow._number + 1;
    ...
    ws.addRow([
      "Material Desciption",
      "Total Qty in Technical",
      "Existing Stock Qty (Smart)",
      "Existing Stock Qty (Ericsson)",
      "Quotation Net Qty",
      dataComm.early_start === true ? "Early Start Qty" : "In PO Qty",
      "Unit Price",
      "Currency",
      "Total Price",
    ]);
    ...
    for (let h = 0; h < prod_type.length; h++) {
      ...
      let phy_item_group = dataPrint.filter(
        (pp) => pp.product_type === prod_type[h]
      );
      let phy_group = [
        ...new Set(phy_item_group.map(({ phy_group }) => phy_group)),
      ];
      const getRowLast = ws.lastRow._number;
      ws.mergeCells("A" + getRowLast + ":I" + getRowLast);
      ws.getCell("A" + getRowLast).fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FFF8F6DF" },
      };
      for (let i = 0; i < phy_group.length; i++) {
        let phy_item = dataPrint.filter(
          (pp) =>
            pp.phy_group === phy_group[i] && pp.product_type === prod_type[h]
        );
        ...
        if (dataComm.early_start === true) {
          for (let j = 0; j < phy_item.length; j++) {
            ws.addRow([
              phy_item[j].package_name,
              phy_item[j].qty_tech,
              phy_item[j].smart_stock,
              phy_item[j].qty_ericsson,
              phy_item[j].qty_comm_quotation,
              phy_item[j].qty_early_start,
              phy_item[j].unit_price,
              phy_item[j].currency,
              phy_item[j].total_price,
            ]);
          }
        } else {
          for (let j = 0; j < phy_item.length; j++) {
    ...
    ws.getCell(
      "I" + getRowLastTotal
    ).value = this.state.total_comm.totalPriceComm;
    ws.getCell("A" + getRowLastTotal).font = { bold: true };
    ...
    saveAs(
      new Blob([comFormat]),
      "Commercial BOQ " + dataComm.no_boq_comm + " Report.xlsx"
    );
  };
                                ...
                                <tr>
                                  ...
                                  >
                                    <Select
                                      cacheOptions
                                      options={
                                        this.state.list_po_data_selection
                                      }
                                      // defaultOptions
                                  ...
                                  >
                                    <Button
                                      ...
                                      style={{ margin: "10px" }}
                                      color="success"
                                      onClick={this.updatePOAssign}
                                      disabled={
                                        this.state.data_po_data_selected ===
                                        null
                                      }
                                    >
                                      Save
