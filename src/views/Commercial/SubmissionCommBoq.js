...
class SubmissionCommBoq extends Component {
  constructor(props) {
    super(props);
    ...
    this.exportSubmissionFormatNew = this.exportSubmissionFormatNew.bind(this);
    this.createNewSubmission = this.createNewSubmission.bind(this);
    this.handleChangeSubmissionNumber = this.handleChangeSubmissionNumber.bind(
      this
    );
  ...
  handleChangeChecklist(e) {
    ...
    this.setState((prevState) => ({
      checkedPackage: prevState.checkedPackage.set(item, isChecked),
    }));
  ...
  handleChangeChecklistAll(e) {
    ...
    if (isChecked === true) {
      for (let x = 0; x < OnlyGroup.length; x++) {
        this.setState((prevState) => ({
          checkedPackage: prevState.checkedPackage.set(OnlyGroup[x], true),
        }));
      }
      ...
      for (let x = 0; x < OnlyGroup.length; x++) {
        this.setState((prevState) => ({
          checkedPackage: prevState.checkedPackage.set(OnlyGroup[x], false),
        }));
      }
  ...
  LoopEarlyQTY(index, dataCommItem, isChecked) {
    if (index >= dataCommItem.length) {
      ...
      this.patchDatatoAPI(
        "/boq_comm_op/" + this.state.boq_comm_API._id,
        dataUpdateComm,
        this.state.boq_comm_API._etag
      ).then((resp) => {});
    } else {
      let dataUpdate = {
      ...
      this.patchDatatoAPI(
        "/boq_comm_items_op/" + dataCommItem[index]._id,
        dataUpdate,
        dataCommItem[index]._etag
      ).then((resp) => {
        if (resp !== undefined) {
          if (resp.status < 400) {
  ...
  saveSmartStockorPrice = async (verNumber, _id_Comm, _etag_Comm) => {
    ...
    for (const [key, value] of AllEdit.entries()) {
      ...
      if (dataIndex !== undefined) {
        let idxSmart = qty_smart.has(key)
          ? parseFloat(this.checkValuetoZero(qty_smart.get(key)))
          : dataIndex.smart_stock;
        let idxErics = qty_ericsson.has(key)
          ? parseFloat(this.checkValuetoZero(qty_ericsson.get(key)))
          : dataIndex.qty_ericsson;
        let idxPrice = edit_price.has(key)
          ? parseFloat(this.checkValuetoZero(edit_price.get(key)))
          : dataIndex.unit_price;
        let idxCurr = curr_item.has(key)
          ? curr_item.get(key)
          : dataIndex.currency;
        let idxInsen = insen_item.has(key)
          ? parseFloat(this.checkValuetoZero(insen_item.get(key)))
          : dataIndex.incentive;
        let total_price =
          (dataIndex.qty_tech - (idxSmart + idxErics)) * idxPrice;
        let net_total =
          (dataIndex.qty_tech - (idxSmart + idxErics)) * idxPrice - idxInsen;
        let dataUpdate = {
        ...
        const respondPatchQTY = await this.patchDatatoAPI(
          "/boq_comm_items_op/" + key,
          dataUpdate,
          dataIndex._etag
        );
        if (respondPatchQTY !== undefined) {
          if (respondPatchQTY.status >= 200 && respondPatchQTY.status < 300) {
            ...
            commItems[commItems.findIndex((e) => e._id === key)]["_etag"] =
              respondPatchQTY.data._etag;
          }
        }
      }
    }
    ...
    for (let j = 0; j < commItems.length; j++) {
      ...
      const respondPatchVersion = await this.patchDatatoAPI(
        "/boq_comm_items_op/" + commItems[j]._id,
        dataVersion,
        commItems[j]._etag
      );
      if (respondPatchVersion !== undefined) {
        if (
          respondPatchVersion.status >= 200 &&
          respondPatchVersion.status < 300
        ) {
    ...
    if (respondStockPrice.length === AllEdit.size) {
      ...
      const respondUpdateComm = await this.patchDatatoAPI(
        "/boq_comm_op/" + _id_Comm,
        updateComm,
        _etag_Comm
      );
      this.setState(
        {
          ...
          action_message: "Your Commercial BOQ has been updated",
        },
        () => {
          this.toggleLoading();
          setTimeout(function () {
  ...
  countPerItemFromDataBase(dataTech) {
    ...
    let get_id_package = [
      ...new Set(onlyItem.map(({ id_pp_doc }) => id_pp_doc)),
    ];
    let countData = {};
    for (let i = 0; i < get_id_package.length; i++) {
      const materialQTY = onlyItem.filter(
        (d) => d.id_pp_doc === get_id_package[i]
      );
  ...
  getAllPP() {
    this.getDatafromAPIEXEL(
      '/pp_sorted_nonpage?projection={"_id" : 1,"pp_id" : 1, "pp_group" : 1, "product_name" :1, "uom" : 1, "physical_group" : 1, "product_type" : 1, "pp_key" : 1, "pp_cust_number" : 1}'
    ).then((resPP) => {
      if (resPP !== undefined) {
  ...
  async getDataCommAPI() {
    let resComm = await this.getDatafromAPI(
      "/boq_comm_audit/" +
        this.props.match.params.id +
        '?embedded={"created_by":1, "updated_by" : 1}'
    );
    if (resComm === undefined) {
      resComm["data"] = undefined;
    }
    if (resComm.data !== undefined) {
      if (resComm.data.early_start !== undefined) {
        this.setState({
          early_start: resComm.data.early_start,
          version_now: resComm.data.version,
          version_selected: resComm.data.version,
        });
      }
      let dataCommItems = [];
      ...
      for (let i = 0; i < getNumberPage; i++) {
        let DataPaginationItems = arrayDataCommItems.slice(
          i * 20,
          (i + 1) * 20
        );
        let arrayIdItems = '"' + DataPaginationItems.join('", "') + '"';
        let where_id_Items =
          '?where={"_id" : {"$in" : [' + arrayIdItems + "]}}";
        let resItems = await this.getDatafromAPI(
          "/boq_comm_items_non_page" + where_id_Items
        );
      ...
      if (dataCommItems.length !== 0) {
        ...
        this.setState({
          commercialData: dataCommItems,
          boq_comm_API_now: resComm.data,
          boq_comm_API: resComm.data,
          curr_rev: resComm.data.version,
          commercialData_now: dataCommItems,
        });
      }
    } else {
      this.setState({
  ...
  async getDataCommercial(_id) {
    ...
    if (getComm.data !== undefined) {
      ...
      this.setState(
        {
          data_comm_boq: dataComm.data,
          data_comm_boq_items: dataComm.data.site,
        },
        () => {
  ...
  getTechBoqData(_id_tech) {
    ...
    this.getDataFromAPINODE(
      "/techBoq/" +
        _id_tech +
        '?v={"techBoqSite.site_id" : 1, "techBoqSite.program" : 1, "techBoqSite.priority" : 1}'
    ).then((res) => {
      if (res.data !== undefined) {
        const dataTech = res.data;
        let listPriority = [
          ...new Set(dataTech.data.techBoqSite.map(({ priority }) => priority)),
        ];
        ...
        this.setState({
          ...
          list_priority_avail: listPrioritySelection,
        });
      }
    });
  }
  ...
  async getDataCommfromTech(boqTechSelect) {
    ...
    const res = await this.getDatafromAPI(
      "/boq_tech_sorted/" + boqTechSelect + embeddedTech
    );
    if (res !== undefined) {
      if (res.data !== undefined) {
        if (res.data.list_of_id_site !== undefined) {
          const sumPackage = this.countPerItemFromDataBase(
            res.data.list_of_id_site
          );
          console.log("sumPackage", sumPackage);
          const resPP = this.state.pp_all;
          const getProject = this;
          const commView = this.dataComm(
            resPP,
            sumPackage,
            res.data.list_of_id_site[0].list_of_site_items
          );
          this.getDatafromAPI(
            "/boq_tech_sorted/" +
              boqTechSelect +
              '?embedded={"list_of_id_boq_comm":1,"list_of_id_boq_comm.list_of_id_item":1}'
          ).then((resTech) => {
            if (resTech !== undefined) {
              if (resTech.data.list_of_id_boq_comm !== undefined) {
                let listPicked = [];
                resTech.data.list_of_id_boq_comm.map((e) =>
                  e.list_of_id_item.map((i) => listPicked.push(i.id_pp_doc))
                );
          ...
          this.setState({
            prev_boq_tech_select: this.state.Boq_Technical_Select,
          });
          this.toggleLoading();
  ...
  dataCommGroup(dataPP, sumPPGroup) {
    ...
    for (let i = 0; i < Object.keys(sumPPGroup).length; i++) {
      let PPindex = dataPP.filter(
        (PP) => PP.pp_group === Object.keys(sumPPGroup)[i]
      );
      const dataGroupComm = {
  ...
  dataComm(dataPP, SummPP, dataPPTech) {
    ...
    for (let i = 0; i < dataPP.length; i++) {
      ...
      let PPTechIndex = dataPPTech.find(
        (PP) => PP.id_pp_doc === Object.keys(SummPP)[i]
      );
      if (PPindex !== undefined) {
        const dataItemsComm = {
          ...
          pp_id: PPindex.pp_id,
          pp_key:
            PPTechIndex.pp_key !== undefined
              ? PPTechIndex.pp_key
              : this.checkValueReturn(PPindex.pp_key, PPindex.pp_id),
          pp_group:
            PPTechIndex.pp_group !== undefined
              ? PPTechIndex.pp_group
              : PPindex.pp_group,
          package_name:
            PPTechIndex.package_name !== undefined
              ? PPTechIndex.package_name
              : PPindex.product_name,
          product_type:
            PPTechIndex.product_type !== undefined
              ? PPTechIndex.product_type
              : PPindex.product_type,
          pp_cust_number:
            PPTechIndex.pp_cust_number !== undefined
              ? PPTechIndex.pp_cust_number
              : this.checkValueReturn(PPindex.pp_cust_number, PPindex.name),
          phy_group:
            PPTechIndex.phy_group !== undefined
              ? PPTechIndex.phy_group
              : PPindex.physical_group,
          ...
          net_total: 0,
          fas_assignment_id:
            PPindex.fas_category !== undefined ? PPindex.fas_category : null,
          total_price:
            Object.values(SummPP)[i] *
            (PPindex.price == null ? 0 : PPindex.price),
          created_by: this.state.userId,
          updated_by: this.state.userId,
          deleted: 0,
        };
  ...
  DataGroupingView(itemsDatas, dataItemsPicked) {
    ...
    if (dataItemsPicked !== undefined && dataItemsPicked.length !== 0) {
      itemsDatas = itemsDatas.filter(
        (i_d) => dataItemsPicked.includes(i_d.id_pp_doc) !== true
      );
    ...
    let total_comm = {
      totalPriceComm: total_price_comm,
      netTotalComm: net_total_comm,
    };
    this.setState({ total_comm: total_comm });
    let OnlyGrouping = [];
    let OnlyTypeGroup = [
      ...new Set(itemsDatas.map(({ product_type }) => product_type)),
    ];
    // OnlyTypeGroup = OnlyTypeGroup.filter(e => e !== "LCM");
    // OnlyTypeGroup.push("LCM");
    for (let i = 0; i < OnlyTypeGroup.length; i++) {
      const group_item = itemsDatas.filter(
        (d) => d.product_type === OnlyTypeGroup[i]
      );
      const dataGroup = {
  ...
  saveCommtoAPI = async () => {
    ...
    let postComm = await this.postDatatoAPINODE("/commBoq/createCommercial", {
      data: this.state.data_tech_boq_selected,
    });
  ...
  saveProjecttoDB = async () => {
    ...
    const respondGetTech = await this.getDatafromAPI(
      "/boq_tech_op/" +
        dataComm.id_boq_tech_doc +
        '?embedded={"list_of_id_site" :1, "list_of_id_boq_comm" :1}'
    );
    console.log("respondGetTech", respondGetTech);
    const respondUpdateTech = await this.patchDatatoAPI(
      "/boq_tech_op/" + respondGetTech.data._id,
      data_Project,
      respondGetTech.data._etag
    );
        ...
        if (list_of_id_boq_comm.length > 1) {
          for (let j = 0; j < list_of_id_boq_comm.length; j++) {
            if (list_of_id_boq_comm[j]._id !== dataComm._id) {
              let respondUpdateCommList = await this.patchDatatoAPI(
                "/boq_comm_op/" + list_of_id_boq_comm[j]._id,
                data_Project,
                list_of_id_boq_comm[j]._etag
              );
            }
          }
        }
  ...
  overwriteDataItems() {
    ...
    if (this.state.rowsXLS.length !== 0) {
      ...
      this.saveSmartStockorPrice(
        getVerNumber.toString(),
        dataComm._id,
        dataComm._etag
      );
    }
  ...
  revisionDataItems = async () => {
    ...
    let dataItems = this.state.commercialData;
    if (
      this.state.version_selected.toString() !==
      this.state.version_now.toString()
    ) {
    ...
    if (
      commDataRev["created_by"] !== null &&
      commDataRev["created_by"] !== undefined
    ) {
    ...
    if (
      commDataRev["updated_by"] !== null &&
      commDataRev["updated_by"] !== undefined
    ) {
    ...
    const respondRevComm = await this.postDatatoAPI(
      "/boq_comm_items_rev",
      dataStoreRev
    );
    const respondRevItem = await this.postDatatoAPI(
      "/boq_comm_rev",
      commDataRev
    );
    ...
    if (this.state.rowsXLS.length !== 0) {
      const respondRevLoop = await this.LoopOverwrite(
        1,
        dataXLS,
        dataItems,
        dataUpdateComm.version
      );
            ...
            this.setState(
              {
                action_status: "success",
                action_message:
                  "Your Commercial BOQ has been save, please Refresh your page",
              },
              () => {
                this.toggleLoading();
              }
      ...
      this.saveSmartStockorPrice(
        dataUpdateComm.version.toString(),
        dataComm._id,
        respondUpdateComm.data._etag
      );
    }
  };
  ...
  async LoopOverwrite(indexXLS, dataNewXLS, dataOld, verNumber) {
    if (indexXLS >= dataNewXLS.length) {
      const resComm = await this.getDatafromAPI(
        "/boq_comm_audit/" +
          this.props.match.params.id +
          '?embedded={"created_by":1, "updated_by" : 1}'
      );
            ...
            this.setState(
              {
                action_status: "success",
                action_message:
                  "Your Commercial BOQ has been updated, please Refresh your page",
              },
              () => {
                this.toggleLoading();
                setTimeout(function () {
      ...
      const findData = dataOld.find(
        (d) =>
          d.pp_id ===
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "package_key")]
      );
      ...
      if (findData !== undefined && dataNewXLS.length > 1) {
        let price = this.checkValuetoZero(
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "unit_price")]
        );
        let qtyCust = this.checkValuetoZero(
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "smart_stock")]
        );
        let qtyEricsson = this.checkValuetoZero(
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "qty_ericsson")]
        );
        let currency = this.checkValue(
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "currency")]
        );
        let incentive = this.checkValuetoZero(
          dataNewXLS[indexXLS][this.getIndex(dataNewXLS[0], "incentive")]
        );
        ...
        const updateData = {
          ...
          incentive: incentive,
          net_total:
            (findData.qty_tech - (qtyCust + qtyEricsson)) * price - incentive,
          version: verNumber.toString(),
        };
        const respondUpdate = this.patchDatatoAPI(
          "/boq_comm_items_op/" + findData._id,
          updateData,
          findData._etag
        );
          ...
          this.setState(
            {
              action_status: "failed",
              action_message:
                "Sorry, There is something error, please check your format data and try again",
            },
            () => {
              this.toggleLoading();
            }
  ...
  editQtyCust = (e) => {
    ...
    let value = e.target.value;
    this.setState((prevState) => ({
      qty_cust: prevState.qty_cust.set(name, value),
    }));
  };
  ...
  editQtyEricsson = (e) => {
    ...
    let value = e.target.value;
    this.setState((prevState) => ({
      qty_ericsson: prevState.qty_ericsson.set(name, value),
    }));
  };
  ...
  editUnitPrice = (e) => {
    ...
    let value = e.target.value;
    this.setState((prevState) => ({
      unit_price: prevState.unit_price.set(name, value),
    }));
  };
  ...
  handleChangeCurrency = (e) => {
    ...
    const value = e.target.value;
    this.setState((prevState) => ({
      currencyChange: prevState.currencyChange.set(name, value),
    }));
  };
  ...
  packageView(packageData, pt, group) {
    let items = [];
    if (packageData.length !== 0) {
      let itemsget = packageData.filter(
        (d) => d.product_type === pt && d.phy_group === group
      );
  ...
  exportCommercialTemplate = async () => {
    ...
    for (let i = 0; i < dataPrint.length; i++) {
      const total_price =
        (dataPrint[i].qty_tech -
          (dataPrint[i].qty_ericsson + dataPrint[i].smart_stock)) *
        dataPrint[i].unit_price;
      let dataIndex = [
        dataPrint[i].pp_id,
        dataPrint[i].package_name,
        dataPrint[i].unit,
        dataPrint[i].qty_tech,
        dataPrint[i].smart_stock,
        dataPrint[i].qty_ericsson,
        dataPrint[i].unit_price,
        dataPrint[i].currency,
        dataPrint[i].qty_tech -
          (dataPrint[i].qty_ericsson + dataPrint[i].smart_stock),
        total_price,
        dataPrint[i].product_type,
      ];
      ws.addRow(dataIndex);
    ...
    saveAs(
      new Blob([comFormat]),
      "Commercial BOQ " + dataComm.no_boq_comm + " Format.xlsx"
    );
  };
  ...
  savePO() {
    ...
    if (
      this.state.po_number_selected.length !== 0 &&
      this.state.po_number_selected !== null
    ) {
      const savePO = {
      ...
      this.patchDatatoAPI(
        ...
        if (res !== undefined) {
          if (res.status < 400) {
            ...
            this.setState({
              boq_comm_API: commAPI,
              action_status: "success",
              action_message: "PO Number has been saved",
            });
          }
        }
      });
      ...
      this.setState({
        action_status: "failed",
        action_message: "Please Select PO Number",
      });
    }
  }
  ...
  getListVersion(_id) {
    this.getDatafromAPI(
      '/boq_comm_rev_all?where={"id_document":"' +
        _id +
        '"}&projection={"version":1, "rev1":1}'
    ).then((res) => {
      console.log("rev", res);
      if (res !== undefined) {
  ...
  formatRev(dataRev) {
    ...
    for (let i = 0; i < dataRev.length; i++) {
      format_rev.push([
        dataRev[i].pp_id,
        dataRev[i].smart_stock,
        dataRev[i].qty_ericsson,
        dataRev[i].unit_price,
      ]);
    }
    this.setState({ format_rev: format_rev, rowsXLS: format_rev });
  ...
  getItemsCommRevAPI(id_comm, version) {
    ...
    const where_id_Comm =
      'where={"id_boq_comm_doc" : "' +
      id_comm +
      '" , "version" : "' +
      version +
      '"}';
    this.getDatafromAPI("/boq_comm_items_rev_all?" + where_id_Comm).then(
      (res) => {
  ...
  handleInputChange = (newValue) => {
    ...
    this.setState({
      data_po_data_selected: { _id: newValue.value, po_number: newValue.label },
    });
    return newValue;
  };
  ...
  saveNote = () => {
    ...
    for (let numbNote = 1; numbNote <= 6; numbNote++) {
      if (numbNote !== 5) {
        updateNote["note_" + numbNote.toString()] = this.state.noteChange[
          parseInt(numbNote)
        ];
        if (
          ...
          updateNote["note_" + numbNote.toString()] = this.checkValuetoString(
            commAPI["note_" + numbNote.toString()]
          );
        }
        updateNote[
          "note_name_" + numbNote.toString()
        ...
        if (
          ...
          updateNote[
            ...
            commAPI["note_name_" + numbNote.toString()]
          );
        }
      }
    }
          ...
          this.setState(
            {
              boq_comm_API: commAPI,
              action_status: "success",
              action_message: "Your Commercial BOQ Note has been updated",
            },
            () => {
              setTimeout(function () {
                window.location.reload();
  ...
  async updatePOAssign() {
    ...
    if (updateComm.data !== undefined) {
      ...
      if (updateComm.response !== undefined) {
        this.setState({
          action_status: "failed",
          action_message: updateComm.response.error,
        });
      } else {
        this.setState({ action_status: "failed" });
      }
  ...
  async deleteSubmission() {
    ...
    let postSubms = await this.patchDatatoAPINODE(
      "/commBoq/cancelCommercialSubmition/" + dataComm._id,
      { commercialSubmission: this.state.submission_number_selected }
    );
    if (postSubms !== undefined)
      if (postSubms.data !== undefined) {
        this.setState({ action_status: "success" });
      } else {
        if (postSubms.response !== undefined) {
          if (postSubms.response.data.error.message !== undefined) {
            this.setState({
              action_status: "failed",
              action_message: postSubms.response.data.error.message,
            });
          } else {
            this.setState({
              action_status: "failed",
              action_message: postSubms.response.data.error,
            });
          }
        } else {
          this.setState({ action_status: "failed" });
  ...
  async createNewSubmission() {
    ...
    const dataComm = this.state.data_comm_boq;
    let postSubms = await this.patchDatatoAPINODE(
      "/commBoq/createCommercialSubmition/" + dataComm._id,
      { towerList: this.state.rowsXLS }
    );
    if (postSubms !== undefined)
      if (postSubms.data !== undefined) {
        this.setState({ action_status: "success" });
  ...
  async exportSubmissionFormatNew() {
    const wb = new Excel.Workbook();
    const ws = wb.addWorksheet();

    const dataComm = this.state.data_comm_boq;
    let dataSites = (dataSites = this.state.data_comm_boq_items);
    let dataSitesNonSubms = this.state.data_comm_boq_items.filter(
      (e) => e.submission_id === undefined || e.submission_id === null
    );
    ...
    for (let i = 0; i < dataSitesNonSubms.length; i++) {
      ws.addRow([
        dataSites[i].site_id,
        this.getPriorityFromTech(
          dataSitesNonSubms[i].site_id,
          dataSitesNonSubms[i].program
        ),
      ]);
    }
    const comFormat = await wb.xlsx.writeBuffer();
    saveAs(
      new Blob([comFormat]),
      "Submission Commercial BOQ " + dataComm.no_comm_boq + " Format.xlsx"
    );
  }
  ...
  handleChangeSubmissionNumber(e) {
    ...
    this.setState({ submission_number_selected: value }, () => {
      this.viewSubmissionCommData(value);
    });
  }

  viewSubmissionCommData(numb) {
    const dataCommSubmissionStstus = this.state.data_comm_boq.comm_boq_submission_doc.find(
      (e) => e.submission_id === numb
    );
    ...
    if (dataCommSubmissionStstus !== undefined) {
      if (dataCommSubmissionStstus.submission_status === "CANCELED") {
        if (dataCommSubmissionStstus.site_list !== undefined) {
          console.log(
            "dataCommSubmissionStstus.site_list",
            dataCommSubmissionStstus.site_list
          );
          dataSites = dataSites.filter((e) =>
            dataCommSubmissionStstus.site_list.includes(e._id)
          );
          console.log(
            "dataCommSubmissionStstus.site_list dataSites",
            dataSites
          );
        }
      } else {
        if (numb === "all") {
          ...
          this.setState({
            data_view_submission_items: dataSites.filter(
              (e) => e.submission_id === undefined || e.submission_id === null
            ),
          });
        } else {
      ...
      this.setState({
        submission_status_selected: dataCommSubmissionStstus.submission_status,
      });
    } else {
      if (numb === "all") {
        this.setState({ data_view_submission_items: dataSites });
  ...
  exportCommercial = async () => {
    ...
    if (this.state.submission_number_selected !== "all") {
      dataSites = this.state.data_comm_boq_items.filter(
        (e) => e.submission_id === this.state.submission_number_selected
      );
    } else {
    ...
    ws.getCell("F5").value = this.state.submission_number_selected;
    ws.getCell("F5").alignment = { horizontal: "left" };
    ...
    ws.mergeCells("A9:I9");
    ws.getCell("A9").value = "Submission BOQ";
    ws.getCell("A9").font = { size: 14, bold: true };
    ws.getCell("A9").alignment = { vertical: "middle", horizontal: "center" };
    ws.getCell("A9").border = { bottom: { style: "double" } };
    ...
    ws.addRow(["Version", dataComm.version, "", "", "", "", "", "", ""]);
    ws.addRow([
      "Technical Ref",
    ...
    if (
      ...
      this.state.userRole.includes("BAM-CommBoq-ViewWithoutPrice") === true
    ) {
      ppIdRow = [
        "Submission ID",
        "Tower ID",
    ...
    for (let i = 0; i < dataSites.length; i++) {
      ...
      for (let j = 0; j < dataSites[i].items.length; j++) {
        if (
          dataSites[i].submission_id === this.state.submission_number_selected
        ) {
          if (
            this.state.userRole.length !== 0 &&
            this.state.userRole.includes("BAM-CommBoq-ViewWithoutPrice") ===
              false
          ) {
            ws.addRow([
              dataSites[i].submission_id,
              dataSites[i].site_id,
              dataSites[i].program,
              dataSites[i].sow,
  ...
  getPriorityFromTech(tower, program) {
    const dataFindTowerPriroty = this.state.data_tech_boq_sites.find(
      (e) => e.site_id === tower && e.program === program
    );
  ...
  priorityToXLSCreation(arrayPriority) {
    if (arrayPriority.length !== 0) {
      ...
      let dataPrioritySiteAll = this.state.data_tech_boq_sites.filter((e) =>
        arrayPriority.includes(e.priority)
      );
      dataPrioritySiteAll = dataPrioritySiteAll.map((e) => e.site_id);
      console.log("arraySites test", dataPrioritySiteAll, arrayPriority);
      console.log(
        "arraySites dataSitesNonSubms",
        dataSitesNonSubms,
        arrayPriority
      );

      let dataSiteNonSubmsPriority = dataSitesNonSubms.filter((e) =>
        dataPrioritySiteAll.includes(e.site_id)
      );
      ...
      if (dataSiteNonSubmsPriority.length === 0) {
        this.setState({
          action_status: "failed",
          action_message:
            "Sorry, All Site in this priority already became submission",
        });
      } else {
        this.setState({ action_status: null, action_message: null });
      }
    ...
    return (
      <div>
                  ...
                  <React.Fragment>
                    ...
                    >
                      ...
                      >
                        ...
                        <DropdownMenu>
                          <DropdownItem header> Submission File</DropdownItem>
                          <DropdownItem onClick={this.exportCommercial}>
                            {" "}
                            <i
                          ...
                          <DropdownItem
                            onClick={this.exportSubmissionFormatNew}
                          >
                            {" "}
                            <i
                              className="fa fa-file-text-o"
                              aria-hidden="true"
                            ></i>
                            Create Submission Commercial Format
                          </DropdownItem>
                        </DropdownMenu>
                      </Dropdown>
                      <Button
                            ...
                            this.props.match.params.id !== undefined && (
                              <React.Fragment>
                                ...
                                >
                                  <input
                                    type="file"
                                    onChange={this.fileHandlerCommercial.bind(
                                      this
                                    )}
                                    style={{
                                      padding: "10px",
                                  ...
                                  >
                                    <Select
                                      ...
                                      isDisabled={
                                        this.state.list_priority_avail.length ==
                                        0
                                      }
                                    />
                                  </div>
                                  <Button
                                    ...
                                    color="primary"
                                    onClick={this.createNewSubmission}
                                    disabled={
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td>
                            <Input
                              type="select"
                              onChange={this.handleChangeSubmissionNumber}
                              value={this.state.submission_number_selected}
                            >
                              <option value="all">All</option>
                              {this.state.data_comm_boq !== null &&
                                this.state.data_comm_boq.comm_boq_submission_doc.map(
                                  (e) =>
                                    e.submission_status === "CANCELED" ? (
                                      <option
                                        style={{ color: "rgba(230,74,25 ,1)" }}
                                        value={e.submission_id}
                                      >
                                        {e.submission_id + "  (CANCELED)"}
                                      </option>
                                    ) : (
                                      <option value={e.submission_id}>
                                        {e.submission_id}
                                      </option>
                                    )
                                )}
                    ...
                    <table style={{ width: "100%", marginBottom: "0px" }}>
                      <tbody>
                        ...
                        {this.state.data_comm_boq !== null &&
                          this.props.match.params.id !== undefined && (
                            <React.Fragment>
                              {this.state.submission_status_selected ===
                              "CANCELED" ? (
                                <tr
                                  style={{
                                    fontWeight: "390",
                          ...
                          <table className="table-header">
                            <tbody>
                              ...
                              <tr
                                style={{ fontWeight: "425", fontSize: "15px" }}
                              >
                                <td style={{ width: "150px" }}>Commercial </td>
                                <td>:</td>
                                <td colspan="2" style={{ paddingLeft: "10px" }}>
                              ...
                              >
                                ...
                                <td>:</td>
                                <td colspan="2" style={{ paddingLeft: "10px" }}>
                                  {this.state.data_comm_boq.version}
                                </td>
                              </tr>
                              <tr
                                style={{ fontWeight: "425", fontSize: "15px" }}
                      ...
                      <tr>
                        ...
                        {this.state.userRole.length !== 0 &&
                          this.state.userRole.includes(
                            "BAM-SubmissionBoq-ViewWithoutPrice"
                          ) === false && (
                            <React.Fragment>
                            ...
                            {this.state.userRole.length !== 0 &&
                              this.state.userRole.includes(
                                ...
                                <React.Fragment>
                                  <td>
                                    {numberWithCommas(
                                      item.net_price_incentive_usd
                                    )}
                                  </td>
                                  <td>
                                    {numberWithCommas(item.net_price_incentive)}
                                  </td>
                                  <td>
                                    {numberWithCommas(
                                      item.total_price_incentive_usd
                                    )}
                                  </td>
                                  <td>
                                    {numberWithCommas(
                                      item.total_price_incentive
                                    )}
                                  </td>
                                  <td>{item.cpo_boq}</td>
                                </React.Fragment>
        ...
        >
          ...
          <ModalFooter>
            <Button color="danger" onClick={this.deleteSubmission}>
              Delete
            </Button>
...
export default connect(mapStateToProps)(SubmissionCommBoq);
