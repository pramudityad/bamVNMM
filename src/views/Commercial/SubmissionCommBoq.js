...
class SubmissionCommBoq extends Component {
  ...
  checkValueReturn(value1, value2){
    ...
    async deleteSubmission(){
      ...
      let postSubms = await this.patchDatatoAPINODE('/commBoq/cancelCommercialSubmition/'+dataComm._id, {"commercialSubmission" : this.state.submission_number_selected});
      ...
      if(postSubms.data !== undefined){
        ...
        if(postSubms.response !== undefined){
          if(postSubms.response.data.error.message !== undefined){
            this.setState({ action_status: 'failed', action_message: postSubms.response.data.error.message });
          }else{
            this.setState({ action_status: 'failed', action_message: postSubms.response.data.error });
          }
        }else{
          this.setState({action_status : 'failed' });
        }
    ...
    async createNewSubmission(){
        ...
        // let dataCommNew = await this.prepareDataforSaveComm(this.state.data_tech_boq_selected);
        const dataComm = this.state.data_comm_boq;
        let postSubms = await this.patchDatatoAPINODE('/commBoq/createCommercialSubmition/'+dataComm._id, {"towerList" : this.state.rowsXLS});
        if(postSubms !== undefined)
        if(postSubms.data !== undefined){
          this.setState({action_status : 'success'});
        }else{
    ...
    exportCommercial = async () =>{
      ...
      ws.addRow(["Commercial Boq",dataComm.no_comm_boq,"","","","", "", "",""]);
        ...
        <div>
          ...
          <Row>
            <Col xl="12">
              <Card>
                <CardHeader>
                  <span style={{lineHeight :'2', fontSize : '17px'}}>Submission Commercial BOQ </span>
                    ...
                    <div className="card-header-actions" style={{ display: 'inline-flex' }}>
                      <Dropdown isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{float : 'right', marginRight : '10px'}}>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownItem onClick={this.exportCommercial}> <i className="fa fa-file-text-o" aria-hidden="true"></i> Submission Report</DropdownItem>
                    ...
                    <table style={{width : '100%', marginBottom : '0px'}}>
                      <tbody>
                        <tr style={{fontWeight : '425', fontSize : '23px'}}>
                          ...
                          {this.props.match.params.id === undefined ? "CREATE" : ""} SUBMISSION COMMERCIAL BOQ
                          </td>
                        </tr>
                        {this.state.data_comm_boq !== null && this.props.match.params.id !== undefined && (
                          <React.Fragment>
                            ...
                            <table className="table-header">
                              <tbody>
                                ...
                                <tr style={{fontWeight : '425', fontSize : '15px'}}>
                                  ...
                                  <td>:</td>
                                  <td colspan="2" style={{paddingLeft:'10px'}}>{this.state.data_comm_boq.no_comm_boq}</td>
                                </tr>
                                <tr style={{fontWeight : '425', fontSize : '15px'}}>
                                  <td>Version</td>
                                  <td>:</td>
          ...
          <Modal isOpen={this.state.modal_delete} toggle={this.toggleDelete} className={'modal-sm ' + this.props.className}>
            <ModalBody>
              ...
              <div style={{textAlign : 'center', margin : '15px 0 20px 0'}}>
                <span style={{fontWeight : '500', fontSize : '15px'}}>Submission</span>
              </div>
              <div style={{textAlign : 'center'}}>
            ...
            <ModalFooter>
              <Button color="danger" onClick={this.deleteSubmission}>Delete</Button>
