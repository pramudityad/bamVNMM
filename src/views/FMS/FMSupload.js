...
class FMSupload extends Component {
  ...
  getDataRolesMilestone() {
    getDatafromAPIEXEL(
      '/rules_sorted_non_page?where={"rule_type": "Erisite POD Upload and Actualization"}'
    ).then((resp) => {
      if (resp !== undefined) {
    ...
    if (!inputValue) {
      ...
      const getWPID = await getDatafromAPIEXEL(
        '/custdel_sorted_non_page?where={"WP_ID":{"$regex":"' +
          ...
          this.state.project_name_selected +
          '"}&projection={"WP_ID":1,"WP_Name":1,"CD_Info_Project_Name":1,"CD_Info_Project":1,"Site_Info_SiteID_NE":1,"Tower_Info_TowerID_NE":1,"Tower_Info_TowerID_NE":1,"Tower_Info_TowerName_NE":1,"Tower_Info_Province_NE":1, "WP_ID_Erisite":1}'
      );
      if (getWPID !== undefined && getWPID.data !== undefined) {
        ...
        getWPID.data._items.map((wp) =>
          wp_id_list.push({
            ...
            Site_Info_SiteID_NE: null,
            Site_Info_SiteID_NE_Actual: wp.Tower_Info_TowerID_NE,
            Site_Info_SiteID_Value_NE: wp.Tower_Info_TowerID_NE,
            Site_Info_SiteName_NE_Actual: wp.Tower_Info_TowerName_NE,
            Activity_Region: wp.Tower_Info_Province_NE,
          })
        );
      }
  ...
  handlemultipleCD = (cdlist) => {
    ...
    if (cdlist !== undefined && cdlist !== null) {
      ...
      this.setState(
        { cd_selected: cd_array, site_selected: site_selected },
