...
class LCCCreation extends Component {
  ...
  async loadOptionsMR(inputValue) {
    if (!inputValue) {
      ...
      let mr_list = [];
      const getMR = await this.getDataFromAPINODE('/matreq?srt=_id:-1&q={"mr_id":{"$regex":"' + inputValue + '", "$options":"i"}}&v={"mr_id":1}');
                  ...
                  <Form>
                    ...
                    <Row>
                      <Col md="3">
                        ...
                        <FormGroup style={{ paddingLeft: "16px" }}>
                          <Label>To {this.state.list_mr_selected.mr_type !== undefined && (this.state.list_mr_selected.mr_type === "New" || this.state.list_mr_selected.mr_type === null) ? "(Site NE)" : "(Warehouse)"}</Label>
                        ...
                        {this.state.list_mr_selected.site_info !== undefined ? this.state.list_mr_selected.site_info[1] !== undefined && this.state.list_mr_selected.mr_type !== "Return" && this.state.list_mr_selected.mr_type !== "Relocation" ? (
                          <FormGroup style={{ paddingLeft: "16px" }}>
                            ...
                            <Input type="text" name="11" value={this.state.list_mr_selected.site_info !== undefined ? this.state.list_mr_selected.site_info[1].site_id : ""} onChange={this.handleChangeForm} readOnly />
                          </FormGroup>
                        ) : (<div></div>) : (<div></div>)}
                      </Col>
                      <Col md="3">
