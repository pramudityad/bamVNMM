...
class CPODatabase extends React.Component {
  ...
  ArrayEmptytoNull(dataXLS) {
    ...
    const idxEM =
      dataXLS.length > 0
        ? dataXLS[0].findIndex((e) => e === "expired_month")
        : 0;
  ...
  saveCPOBulkWithDetail = async () => {
    ...
    const res = await this.postDatatoAPINODE("/cpodb/createCpoDbWithDetail", {
      poData: cpobulkXLS,
    });
    if (res.data !== undefined) {
      this.setState({ action_status: "success", action_message: null });
  ...
  async getPODataListAll() {
    ...
    const res = await this.getDatafromAPINODE(
      "/cpodb/getCpoDb?noPg=1&srt=_id:-1&q={" + po_number + queryDate + "}"
    );
  ...
  exportCPODBList = async () => {
    ...
    for (let i = 0; i < dataPOAll.length; i++) {
      ws.addRow([
        dataPOAll[i].po_number,
        convertDateFormat(dataPOAll[i].date),
        this.Aging(convertDateFormat(dataPOAll[i].date)),
        dataPOAll[i].expired_date !== null &&
        dataPOAll[i].expired_date !== undefined
          ? convertDateFormat(dataPOAll[i].expired_date)
          : null,
        dataPOAll[i].currency,
        dataPOAll[i].payment_terms,
        dataPOAll[i].shipping_terms,
        dataPOAll[i].contract,
        dataPOAll[i].contact,
      ]);
    }
                  ...
                  <div style={{ marginRight: "10px" }}>
                    ...
                    >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownItem onClick={this.exportFormatCPO_level1}>
                          {" "}
                          CPO Level 1 Template
                        </DropdownItem>
                        <DropdownItem onClick={this.exportFormatCPOWithDetail}>
                          {" "}
                          CPO With Detail Template
                        </DropdownItem>
                        <DropdownItem onClick={this.exportCPODBList}>
                          {" "}
                          CPO List Report
                        </DropdownItem>
                      </DropdownMenu>
                ...
                <Card style={{ margin: "10px 10px 5px 10px" }}>
                  ...
                  <CardFooter>
                    ...
                    <Button
                      ...
                      size="sm"
                      disabled={this.state.rowsXLS.length === 0}
                      onClick={this.saveCPOBulkWithDetail}
                    >
                      {" "}
                      <i className="fa fa-save" aria-hidden="true">
                        {" "}
                          ...
                          {this.state.po_op_data.map((po) => (
                            <React.Fragment key={po._id + "frag"}>
                              ...
                              >
                                ...
                                {this.Aging(convertDateFormat(po.date)) >=
                                120 ? (
                                  <td
                                    style={{
                                      textAlign: "center",
                                ...
                                <td style={{ textAlign: "center" }}>
                                  {po.expired_date !== null &&
                                  po.expired_date !== undefined
                                    ? convertDateFormat(po.expired_date)
                                    : null}
                                </td>
                                <td style={{ textAlign: "center" }}>
                                  {po.currency}
                                ...
                                <td style={{ textAlign: "center" }}>
                                  <Link
                                    to={"/detail-list-cpo-database/" + po._id}
                                  >
                                    <Button
                                      color="primary"
                                      size="sm"
