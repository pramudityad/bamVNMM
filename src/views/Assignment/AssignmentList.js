...
class AssignmentList extends Component {
  ...
  async downloadAllAssignment() {
    ...
    let headerRow = [
      ...
      "identifier",
      "Current Status",
      "SH Assignment Created On",
      "SH Assignment Created By",
      "ssow_rbs_id_1",
    ...
    for (let i = 0; i < allAssignmentList.length; i++) {
      const creator_asg_mitt = allAssignmentList[i].ASP_Assignment_Status.find(
        (e) => e.status_value === "CREATED"
      );
      let rowAdded = [
        allAssignmentList[i].Assignment_No,
        allAssignmentList[i].SH_Assignment_No,
        allAssignmentList[i].cust_del !== undefined
          ? allAssignmentList[i].cust_del.map((cd) => cd.cd_id).join(", ")
          : allAssignmentList[i].cd_id,
        ...
        creator_asg_mitt !== undefined ? creator_asg_mitt.status_date : null,
        creator_asg_mitt !== undefined ? creator_asg_mitt.status_updater : null,
      ];
  ...
  async downloadAllAssignmentBAST() {
    ...
    let getASG = await this.getDataFromAPINODE(
      '/aspAssignment/aspassign?srt=_id:-1&noPg=1&q={"PO_Number" : {"$ne" : null}}'
    );
    if (getASG.data !== undefined) {
      allAssignmentList = getASG.data.data;
    }
