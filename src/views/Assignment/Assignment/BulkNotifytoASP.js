...
class BulkNotifytoASP extends Component {
  ...
  getAssignmentList() {
    ...
    filter_array.push(
      '"$or":[{"Current_Status" : "ASP ASSIGNMENT CREATED"}, {"Current_Status" : "PM APPROVE"}]'
    );
  ...
  getAssignmentListAll() {
    ...
    const maxPage = this.state.perPage;
    this.getDataFromAPINODE(
      '/aspAssignment/aspassign?srt=_id:-1&q={ "$or" :[{"Current_Status" : "ASP ASSIGNMENT CREATED"}, {"Current_Status" : "PM APPROVE"}]}'
    ).then((res) => {
      if (res.data !== undefined) {
        const items = res.data.data;
        const totalData = res.data.totalResults;
  ...
  async requestForNotifyBulk() {
    ...
    this.patchDatatoAPINODE("/aspAssignment/notifyManyAspAssignment", {
      ids: sucPatch,
    }).then((res) => {
      if (res.data !== undefined) {
