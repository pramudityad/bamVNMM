...
class BulkAssignment extends Component {
  constructor(props) {
    ...
    this.state = {
        ...
        redirectSign : false,
        asp_list : [],
        uploadan_type : "no Predefined SSOW",
    };
  ...
  async checkingDataAssignment(dataXLS){
    ...
    let wp_invalid = [];
    const dataXLSASG = {
      "includeSsow" : this.state.uploadan_type === "no Predefined SSOW" ? true : false,
      "data" : dataXLS
    }
  ...
  exportFormatBulkAssignment = async () =>{
    ...
    if(this.state.uploadan_type === "no Predefined SSOW"){
      let headerRow = ["id","project","sow_type", "created_based", "vendor_code","vendor_name","payment_terms","identifier"];
      if(sow_type === "RBSTRM"){
          ...
          <CardBody className='card-UploadBoq'>
            ...
            <table style={{width : '100%', marginBottom : '0px', fontSize : '20px', fontWeight : '500'}}>
              <tbody>
                ...
                <tr>
                  <td colSpan="4" style={{textAlign : 'center', fontSize : '15px', color : 'red'}}>{this.state.uploadan_type === "no Predefined SSOW" ? "It will need approval from authoried": "SSOW List get from default mapping of SSOW to CD ID" }</td>
                </tr>
              </tbody>
            </table>
