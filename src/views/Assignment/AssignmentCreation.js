...
class AssignmentCreation extends Component {
  ...
  handleChangeActivity = async (newValue) => {
    let dataActivity = this.state.list_activity_selection.find(
      (e) => e._id === newValue.value
    );
    this.setState({
      activity_selected: newValue.value,
      list_activity_selected: dataActivity,
    });
    const getProject = await this.getDataFromAPI(
      "/project_sorted/" + dataActivity.CD_Info_Project
    );
    ...
    dataForm[4] =
      dataActivity.Site_Info_Latitude_NE !== ""
        ? dataActivity.Site_Info_Latitude_NE
        : "0.0";
    dataForm[5] =
      dataActivity.Site_Info_Longitude_NE !== ""
        ? dataActivity.Site_Info_Longitude_NE
        : "0.0";
    dataForm[6] = dataActivity.Site_Info_SiteID_FE;
    dataForm[7] = dataActivity.Site_Info_SiteName_FE;
    dataForm[8] =
      dataActivity.Site_Info_Latitude_FE !== ""
        ? dataActivity.Site_Info_Latitude_FE
        : "0.0";
    dataForm[9] =
      dataActivity.Site_Info_Longitude_FE !== ""
        ? dataActivity.Site_Info_Longitude_FE
        : "0.0";
  ...
  async loadOptionsActivity(inputValue) {
    if (!inputValue) {
      ...
      let activity_list = [];
      const getActivity = await this.getDataFromAPI(
      ...
      if (getActivity !== undefined && getActivity.data !== undefined) {
        this.setState(
          { list_activity_selection: getActivity.data._items },
          () =>
            getActivity.data._items.map((activity) =>
              activity_list.push({
                label:
                  activity.WP_ID !== undefined
                    ? activity.WP_ID + " (" + activity.WP_Name + ")"
                    : null,
                value: activity._id,
              })
            )
  ...
  loadOptionsASP() {
    this.getDataFromAPIEXEL(
      '/vendor_data_non_page?where={"invalid":{"$ne":true}}'
    ).then((res) => {
      ...
      if (dataChecking[0].operation === "INVALID") {
        ...
        if (
          ...
          if (this.state.can_edit_ssow === true) {
            ...
            let linkImp =
              "https://bam-id.e-dpm.com/assignment-detail/" + response._id;
            const bodyEmail =
              "<h2>DPM - BAM Notification</h2><br/><span>Please be notified that the following Assignment has been created and need approval because the assingment not using list SSOW from the mapping, <br/><br/><i>Site</i>: <b>" +
              response.Site_ID +
              "</b> <br/><i>Project</i>: <b>" +
              response.Project +
              "</b><br/><i>Assignment</i>: <b>" +
              response.Assignment_No +
              "</b><br/><br/>is created by " +
            ...
            let dataEmail = {
              ...
              to: cpm_email + " ;",
              subject:
                "[Assignment Need Approval] Assignment " +
                response.Assignment_No,
              body: bodyEmail,
            };
  ...
  handleChangeProjectXL(newValue) {
    if (newValue !== null) {
      let obj = this.state.list_project.find(
        (a) => a.Project === newValue[newValue.length - 1].value
      );
      let email = obj.Email_CPM_Name;
      this.setState({
        project_selected: newValue.map((ps) => ps.value).join("///"),
        project_name_selected: newValue.map((ps) => ps.value).join("///"),
        email_cpm: email,
      });
    } else {
      this.setState({
  ...
  handleChangeTowerXL(e) {
    ...
    if (this.state.identifier_by === "cd_id") {
      ...
      if (findCDID !== undefined) {
        ...
        let obj = this.state.list_project.find(
          (a) => a.Project === findCDID.CD_Info_Project_Name
        );
        let email = null;
        if (obj !== undefined) {
  ...
  async loadOptionsTowerID(inputValue) {
    if (!inputValue || inputValue.length < 3) {
      ...
      const getTowerID = await this.getDataFromAPIEXEL(
  ...
  async loadOptionsCDID(inputValue) {
    if (!inputValue) {
      ...
      let wp_id_list = [];
      // const getSSOWID = await this.getDataFromAPIEXEL('/ssow_sorted_nonpage?where={"ssow_id":{"$regex":"'+inputValue+'", "$options":"i"}, "sow_type":"'+this.state.list_activity_selected.CD_Info_SOW_Type +'"}');
      const getWPID = await this.getDataFromAPIEXEL(
        '/custdel_sorted_non_page?where={"WP_ID":{"$regex":"' +
          inputValue +
          '", "$options":"i"}}&projection={"WP_ID":1,"WP_Name":1,"CD_Info_Project_Name":1,"CD_Info_Project":1}'
      );
            ...
            <Card>
              ...
              <CardBody>
                <Form>
                    ...
                    <Col md="6">
                      ...
                      <FormGroup style={{ paddingLeft: "16px" }}>
                        ...
                        <Input
                          ...
                          onChange={this.handleChangeForm}
                          value={
                            this.state.create_assignment_form[15] === null
                              ? ""
                              : this.state.create_assignment_form[15]
                          }
                        >
                  ...
                  {this.state.assignment_ssow_upload !== null &&
                    this.state.assignment_ssow_upload !== undefined &&
                    this.state.identifier_by === "Tower ID" && (
                      <Row>
                        <Col md="6">
                          <FormGroup style={{ paddingLeft: "16px" }}>
                            <Label>CD ID</Label>
                            <Input
                              ...
                              name="po_line_item"
                              value={
                                this.state.assignment_ssow_upload.cust_del !==
                                undefined
                                  ? this.state.assignment_ssow_upload.cust_del.map(
                                      (e) => e.cd_id + "; "
                                    )
                                  : ""
                              }
                  ...
                  <span>
                    Please click check button for checking the form before save
                    it and for checking availibilty of default SSOW
                  </span>
                ...
                <span style={{ color: "red" }}>
                  Editable SSOW Need Approval from the authorized on the
