...
class CPODatabaseDetail extends Component {
  ...
  // async getPODataList(_id){
  //   let getComm = await this.getDataFromAPINODE('/cpodb/getCpoDb/'+_id)
  //   console.log('get CPOdb lvl 2', getComm.data)
  ...
  saveCPO2Bulk = async () => {
    ...
    console.log(cpobulkXLS);
    console.log(_id);
    const cpoPost = this.postDatatoAPINODE('/cpodb/createCpoDbDetail/'+_id, { 'detailData': cpobulkXLS })
      .then(res => {
        console.log('res lvl 2', res);
        if (res.data !== undefined) {
                        ...
                        <table className="table-header">
                          <tbody>
                            ...
                            <tr style={{ fontWeight: '425', fontSize: '15px' }}>
                              ...
                              <td>:</td>
                              <td>{this.state.data_cpo.po_number}</td>
                              {/* project_name */}
